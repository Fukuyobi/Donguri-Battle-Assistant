...
// @match        https://donguri.world/arena
// @match        https://donguri.world/arenalogs
...
(function() {
  ...
  function dbaIsRbLiveMapStaticUrl(url){
    try{
      ...
      if(rawFetch){
        window.fetch = async function(){
          ...
          try{
            ...
            if(dbaIsRbLiveMapStaticUrl(reqUrl) && resp && typeof resp.clone === 'function'){
              resp.clone().json().then((payload) => {
                try{
                  handleRbLiveMapPayloadFromOfficial(
                    payload,
                    'fetch:/map/rb/static'
                  );
                }catch(_e){}
  ...
  // =========================
  // ドメイン共通 Storage
  //  - donguri.5ch.io と donguri.world は同一ゲームサーバーの別入口なので、
  //    Tampermonkey/Violentmonkey 側のスクリプト保存領域を優先して共有する。
  //  - 旧版で origin 別 localStorage に保存済みの値も読めるよう、未保存時は localStorage を fallback する。
  // =========================
  function dbaStorageGetItem(key){
    const k = String(key || '');
    if(!k) return null;
    try{
  ...
  function dbaStorageSetItem(key, value){
    const k = String(key || '');
    if(!k) return;
  ...
  function dbaStorageRemoveItem(key){
    const k = String(key || '');
    if(!k) return;
    try{
  ...
  // =========================
  // どんぐりアリーナログ / どんぐりネーム関連 Storage
  // ※ /arenalogs 分岐より前に置く
  //    （/arenalogs ではこの後の早期 return があるため）
  // =========================
  const LS_DONGURI_NAME_KEY = 'dba.profile.donguriName.v1'; // string
  const LS_ARENALOGS_SELF_ONLY_KEY = 'dba.arenalogs.selfOnly.v1'; // 0/1
  ...
  function loadStoredDonguriName(){
    try{
      const raw = dbaStorageGetItem(LS_DONGURI_NAME_KEY);
  ...
  function saveStoredDonguriName(name){
    try{
      const s = sanitizeText(name || '');
      if(s){
        dbaStorageSetItem(LS_DONGURI_NAME_KEY, s);
      }
    }catch(_e){}
  }

  function loadArenaLogsSelfOnlyEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_ARENALOGS_SELF_ONLY_KEY);
  ...
  function saveArenaLogsSelfOnlyEnabled(on){
    try{
      dbaStorageSetItem(LS_ARENALOGS_SELF_ONLY_KEY, on ? '1' : '0');
    }catch(_e){}
  }
  ...
  function findArenaLogsBattleCardTable(doc){
    if(!doc) return null;
    const tables = Array.from(doc.querySelectorAll('table'));
    for(const table of tables){
      const heads = Array.from(table.querySelectorAll('thead th')).map((th) => sanitizeText(th.textContent || ''));
      if(heads.includes('時間') && heads.includes('対戦カード') && heads.includes('結果')){
        return table;
      }
    }
    return null;
  ...
  function arenaBattleCardContainsSelf(cardText, selfName){
    const card = sanitizeText(cardText || '').toLowerCase();
    const self = sanitizeText(selfName || '').toLowerCase();
    if(!card || !self) return false;
    return card.includes(self);
  }

  function applyArenaLogsSelfFilter(table, selfName, enabled){
    const tbody = table ? (table.tBodies && table.tBodies[0]) : null;
    const rows = tbody ? Array.from(tbody.rows) : [];
    let shown = 0;
    for(const tr of rows){
      const cells = tr ? tr.cells : null;
      const cardText = cells && cells[1] ? cells[1].textContent : '';
      const visible = !enabled || arenaBattleCardContainsSelf(cardText, selfName);
  ...
  async function initArenaLogsSelfFilter(){
    const table = findArenaLogsBattleCardTable(document);
    if(!table) return false;

    if(!document.getElementById('dba-arenalogs-self-filter-style')){
      const style = document.createElement('style');
      style.id = 'dba-arenalogs-self-filter-style';
      style.textContent = [
        '#dba-arenalogs-self-filter{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;margin:0 auto 10px;padding:8px 12px;max-width:fit-content;background:#fff;border:1px solid #00000022;border-radius:12px;box-sizing:border-box;}',
        '#dba-arenalogs-self-filter-main{display:inline-flex;align-items:center;gap:8px;font-weight:700;user-select:none;}',
        '#dba-arenalogs-self-filter input[type="checkbox"]{margin:0;width:18px;height:18px;cursor:pointer;}',
        '#dba-arenalogs-self-filter-text{display:inline-block;cursor:default;}',
        '#dba-arenalogs-self-filter-note{font-size:0.95em;line-height:1.35;color:#333;}'
      ].join('');
      (document.head || document.documentElement).appendChild(style);
    }

    let wrap = document.getElementById('dba-arenalogs-self-filter');
    if(!wrap){
      wrap = document.createElement('div');
      wrap.id = 'dba-arenalogs-self-filter';

      const main = document.createElement('div');
      main.id = 'dba-arenalogs-self-filter-main';

      const check = document.createElement('input');
      check.type = 'checkbox';
      check.id = 'dba-arenalogs-self-filter-check';
      check.checked = loadArenaLogsSelfOnlyEnabled();

      const text = document.createElement('span');
      text.id = 'dba-arenalogs-self-filter-text';
      text.textContent = '自分だけ抽出';

      const note = document.createElement('span');
      note.id = 'dba-arenalogs-self-filter-note';
      ...
      check.addEventListener('change', () => {
        ...
        saveArenaLogsSelfOnlyEnabled(on);
        refresh({ forceNameFetch: on }).catch(() => {});
      });
    }

    const check = document.getElementById('dba-arenalogs-self-filter-check');
    const note = document.getElementById('dba-arenalogs-self-filter-note');

    async function refresh(opts){
      const forceNameFetch = !!(opts && opts.forceNameFetch);
      const wanted = !!(check && check.checked);

      if(note){
        note.textContent = wanted ? 'どんぐりネーム確認中…' : '';
      }

      let selfName = '';
      if(forceNameFetch){
        selfName = await ensureStoredDonguriName();
      }else{
        selfName = await resolveArenaLogsSelfFilterName(wanted);
      }

      const enabled = !!(wanted && selfName);
      const result = applyArenaLogsSelfFilter(table, selfName, enabled);
      ...
      if(note){
        if(wanted){
          if(!selfName){
            note.textContent = 'どんぐりネーム未取得（どんぐり基地から自動取得も試みました）';
          }else{
            note.textContent = `${selfName}：${result.shown}/${result.total}件を表示`;
          }
        }else{
          if(selfName){
            note.textContent = `${selfName}：${result.total}件を対象`;
          }else{
            note.textContent = '';
          }
        }
    ...
    refresh({ forceNameFetch:false }).catch(() => {
      if(note){
        note.textContent = 'どんぐりネーム未取得';
      }
    });
    return true;
  }

  // どんぐりアリーナログ：「自分だけ抽出」だけを挿入
  if(isArenaLogsPage){
    const run = () => {
      try{
        Promise.resolve(initArenaLogsSelfFilter()).catch(() => {});
      }catch(_e){}
    };
    if(document.readyState === 'loading'){
      document.addEventListener('DOMContentLoaded', run, { once:true });
  ...
  const CSS = `
    ...
    /* ===== どんぐりアリーナ：ユーティリティセクション ===== */
    #dba-arena-utility-section {
      position: fixed;
      top: 0;
      left: 0;
      right: 0;
    ...
    #dba-arena-utility-section .dba-arena-util__title {
      font-size: 1.1em;
      font-weight: 900;
      line-height: 1.2;
      text-align: left;
    ...
    #dba-arena-utility-section .dba-arena-util__row {
      display: flex;
      align-items: center;
      justify-content: flex-start;
      flex-wrap: wrap;
    ...
    #dba-arena-utility-section .dba-arena-util__termBar {
      height: 20px;
      background: #ccc;
      border-radius: 10px;
      font-size: 15px;
    ...
    #dba-arena-utility-section .dba-arena-util__termBar > div {
      height: 100%;
      background: #428bca;
      width: 0%;
      text-align: right;
    ...
    #dba-arena-utility-section .dba-arena-util__btnRow {
      display: flex;
      align-items: center;
      justify-content: flex-start;
      flex-wrap: wrap;
    ...
    html.dba-has-arena-util body {
      padding-top: calc(var(--dba-arena-util-height, 126px) + 4px) !important;
    }
    html.dba-has-arena-util body > header {
    ...
    /* 「Reset」ボタン */
    #dba-top-progress-reset-sync,
    #dba-arena-top-progress-reset-sync {
      margin-left: 0;
      flex: 0 0 auto;
    }
    #dba-top-progress-reset-sync[disabled],
    #dba-arena-top-progress-reset-sync[disabled] {
      opacity: 0.45;
      cursor: not-allowed;
      filter: grayscale(1);
    }
    ...
    /* ===== アリーナリザルトモーダル（/arena の「リーダーに挑む」結果表示） ===== */
    #dba-m-arena-result.dba-m-std {
      width: min(900px, calc(100vw - 24px));
      height: min(92svh, calc(100svh - 24px));
      max-height: min(92svh, calc(100svh - 24px));
      overflow: hidden;
    }
    #dba-m-arena-result.dba-m-std .dba-modal__top {
      padding: 8px 10px;
    }
    #dba-m-arena-result.dba-m-std .dba-modal__title {
      font-size: 1.08em;
      font-weight: 800;
      line-height: 1.2;
    }
    #dba-m-arena-result.dba-m-std .dba-modal__mid {
      flex: 1 1 auto;
      min-height: 0;
      max-height: none;
      padding: 10px;
    ...
    #dba-m-arena-result.dba-m-std .dba-modal__bot {
      min-height: 0;
      padding: 8px 10px 10px;
      gap: 8px;
      flex-wrap: wrap;
    }
    #dba-m-arena-result.dba-m-std .dba-btn-apply,
    #dba-m-arena-result.dba-m-std .dba-btn-close {
      margin: 2px 6px;
      padding: 8px 12px;
      font-size: 0.95em;
    }
    .dba-arena-result-text {
      white-space: pre-wrap;
      overflow-wrap: anywhere;
      word-break: break-word;
      line-height: 1.25em;
    ...
    /* ===== アリーナセンターモーダル（/arena の「アリーナを強化/弱体化する」表示） ===== */
    #dba-m-arena-center.dba-m-std {
      width: min(900px, calc(100vw - 24px));
      height: min(92svh, calc(100svh - 24px));
      max-height: min(92svh, calc(100svh - 24px));
      overflow: hidden;
    }
    #dba-m-arena-center.dba-m-std .dba-modal__top {
      padding: 8px 10px;
    }
    #dba-m-arena-center.dba-m-std .dba-modal__title {
      font-size: 1.08em;
      font-weight: 800;
      line-height: 1.2;
    }
    #dba-m-arena-center.dba-m-std .dba-modal__mid {
      flex: 1 1 auto;
      min-height: 0;
      max-height: none;
      padding: 10px;
    ...
    #dba-m-arena-center.dba-m-std .dba-modal__bot {
      min-height: 0;
      padding: 8px 10px 10px;
      gap: 8px;
      flex-wrap: wrap;
    }
    #dba-m-arena-center.dba-m-std .dba-btn-close {
      margin: 2px 6px;
      padding: 8px 12px;
      font-size: 0.95em;
    }
    .dba-arena-center-content {
      width: 100%;
      max-width: 640px;
      margin: 0 auto;
      color: #000;
    ...
    .dba-arena-center-content header {
    ...
    .dba-arena-center-content header h1 {
      display: none !important;
    }
    .dba-arena-center-content header p {
      margin: 0 auto 8px;
    }
    .dba-arena-center-content img {
      max-width: min(250px, 90%);
      height: auto;
    }
    .dba-arena-center-content form {
      margin: 0 auto !important;
      max-width: 100%;
      text-align: center;
    }
    .dba-arena-center-content input[type="text"] {
      width: min(14em, 90%);
      box-sizing: border-box;
      padding: 8px 10px;
    ...
    .dba-arena-center-content input[type="submit"] {
      appearance: none;
      width: fit-content;
      max-width: 100%;
      border: 2px solid #000;
    ...
    .dba-arena-center-content input[type="submit"]:hover {
      filter: brightness(0.97);
      box-shadow: 0 2px 6px rgba(0,0,0,0.16);
    }
    .dba-arena-center-content input[type="submit"]:active {
      transform: translateY(1px);
      box-shadow: 0 1px 2px rgba(0,0,0,0.12);
    }
    .dba-arena-center-plain {
      white-space: pre-wrap;
      overflow-wrap: anywhere;
      word-break: break-word;
      line-height: 1.35em;
    ...
    /* ===== アリーナセンター結果ダイアログ（/arenavol/ のプレーンテキスト表示） ===== */
    #dba-m-arena-center-result.dba-m-alert {
      width: min(520px, calc(100vw - 24px));
    }
    #dba-m-arena-center-result .dba-alert__mid {
      max-height: min(60svh, calc(100svh - 160px));
      overflow: auto;
      scrollbar-gutter: stable both-edges;
      overscroll-behavior: contain;
      -webkit-overflow-scrolling: touch;
    ...
    /* ===== RB：建築物（要塞／レーダー）軽注意ダイアログ ===== */
    #dba-m-rb-build-notice {
      position: fixed;
      left: 0;
      top: 0;
    ...
    #dba-m-rb-build-notice[data-open="1"] {
      display: block;
    }
    #dba-m-rb-build-notice .dba-rb-build-notice__mid {
    ...
    #dba-m-rb-build-notice .dba-rb-build-notice__bot {
      display: flex;
      justify-content: center;
      padding: 8px 12px 12px;
    }
    #dba-m-rb-build-notice .dba-rb-build-notice__ok {
      appearance: none;
      min-width: 72px;
      border: 2px solid #000;
      border-radius: 10px;
    ...
    #dba-m-rb-build-notice .dba-rb-build-notice__ok:hover {
      filter: brightness(0.97);
    }
    #dba-m-rb-build-notice .dba-rb-build-notice__ok:active {
      transform: translateY(1px);
    }
    ...
    .dba-setting-cellsize-list {
      display: inline-grid;
      grid-template-columns: minmax(8em, auto) minmax(6.5em, auto) minmax(6.5em, auto) minmax(7em, auto);
      gap: 4px;
      align-items: center;
    ...
    .dba-setting-cellsize-head {
      font-weight: 700;
      text-align: center;
      white-space: nowrap;
    }
    .dba-setting-cellsize-head--mode {
      text-align: left;
    }
    ...
    .dba-setting-cellsize-inputwrap {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      white-space: nowrap;
    ...
    .dba-setting-cellsize-reset {
      display: flex;
      align-items: center;
      justify-content: center;
    }
    .dba-setting-cellsize-reset .dba-btn-mini {
      margin: 0;
    }
    @media (max-width: 640px) {
      .dba-setting-cellsize-list {
        display: grid;
        grid-template-columns: minmax(6.5em, 1.3fr) minmax(5.5em, 1fr) minmax(5.5em, 1fr) minmax(6.5em, auto);
        gap: 6px 8px;
        width: 100%;
      }
      .dba-setting-cellsize-inputwrap input[type="number"] {
        width: 64px;
      }
    }
    ...
    .dba-ae-tile-op-row .dba-ae-pop-btn {
      margin: 0;
      min-width: 0;
    }
    .dba-ae-tile-op-btn[disabled] {
    ...
    .dba-ae-tile-op-btn[disabled]:hover {
      filter: grayscale(1);
    }
    .dba-ae-tile-op-btn[disabled]:active {
      transform: none;
    }
    /* ===== オート装備ポップアップ：タイル操作の誤クリック防止 ===== */
  ...
  function longPressGaugeStart(x, y, durationMs){
    ...
    const tick = () => {
      if(seq !== DBA_LP_GAUGE.seq) return;
  ...
  const LS_BR_AUTOCLOSE_KEY = 'dba.battleResult.autoClose.enabled.v1'; // 0/1 戦闘ログ自動クローズ
  const LS_BR_AUTOCLOSE_SEC_KEY = 'dba.battleResult.autoClose.sec.v1'; // number 秒
  ...
  const LS_TOP_PROGRESS_RESET_UNLOCK_AT_KEY = 'dba.topElapsedProgress.resetUnlockAt.v1';
  ...
  const LS_PICK_HIDE_LEGACY_PREFIX = 'dba.pick.hideLegacy.v1.'; // weapon / armor
  ...
  function loadPickHideModZero(kind){
    try{
      const raw = dbaStorageGetItem(makePickHideModZeroKey(kind));
  ...
  function savePickHideModZero(kind, on){
    try{
      dbaStorageSetItem(makePickHideModZeroKey(kind), on ? '1' : '0');
    }catch(_e){}
  }

  function makePickHideLegacyKey(kind){
    const k = (kind === 'weapon' || kind === 'armor') ? kind : 'unknown';
    return `${LS_PICK_HIDE_LEGACY_PREFIX}${k}`;
  }

  function loadPickHideLegacy(kind){
    try{
      if(kind !== 'weapon' && kind !== 'armor') return false;
      const raw = dbaStorageGetItem(makePickHideLegacyKey(kind));
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function savePickHideLegacy(kind, on){
    try{
      if(kind !== 'weapon' && kind !== 'armor') return;
      dbaStorageSetItem(makePickHideLegacyKey(kind), on ? '1' : '0');
    }catch(_e){}
  }

  function loadTopSyncInfoShow(){
    try{
      const raw = dbaStorageGetItem(LS_TOP_SYNCINFO_SHOW_KEY);
  ...
  function saveTopSyncInfoShow(on){
    try{
      dbaStorageSetItem(LS_TOP_SYNCINFO_SHOW_KEY, on ? '1' : '0');
    }catch(_e){}
  }

  function loadShowOriginalHeader(){
    try{
      const raw = dbaStorageGetItem(LS_SHOW_ORIGINAL_HEADER_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveShowOriginalHeader(on){
    try{
      dbaStorageSetItem(LS_SHOW_ORIGINAL_HEADER_KEY, on ? '1' : '0');
    }catch(_e){}
  }

  function loadShowLightRefreshButton(){
    try{
      const raw = dbaStorageGetItem(LS_SHOW_LIGHT_REFRESH_BUTTON_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveShowLightRefreshButton(on){
    try{
      dbaStorageSetItem(LS_SHOW_LIGHT_REFRESH_BUTTON_KEY, on ? '1' : '0');
    }catch(_e){}
  }
  ...
  function loadTopElapsedProgress(){
    try{
      const raw = dbaStorageGetItem(LS_TOP_ELAPSED_PROGRESS_KEY);
      if(!raw) return null;
  ...
  function saveTopElapsedProgress(pct, term){
    try{
      ...
      dbaStorageSetItem(LS_TOP_ELAPSED_PROGRESS_KEY, JSON.stringify(obj));
    }catch(_e){}
  }

  function loadTopProgressResetUnlockAt(){
    try{
      const raw = dbaStorageGetItem(LS_TOP_PROGRESS_RESET_UNLOCK_AT_KEY);
      const n = Number(raw);
  ...
  function saveTopProgressResetUnlockAt(unlockAt){
    try{
      const n = Number(unlockAt);
      if(Number.isFinite(n) && n > Date.now()){
        dbaStorageSetItem(LS_TOP_PROGRESS_RESET_UNLOCK_AT_KEY, String(Math.floor(n)));
      }else{
        dbaStorageRemoveItem(LS_TOP_PROGRESS_RESET_UNLOCK_AT_KEY);
      }
    }catch(_e){}
  }

  function isTopProgressResetLocked(now){
    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    const unlockAt = loadTopProgressResetUnlockAt();
    if(!(unlockAt > currentNow)){
      if(unlockAt > 0){
        saveTopProgressResetUnlockAt(0);
      }
      return false;
    }
    return true;
  ...
  const DBA_TOP_PROGRESS_RESET_STATE = {
    unlockTimer: 0,
    lastDisabled: null
  };

  function scheduleTopProgressResetUnlockCheck(){
    if(DBA_TOP_PROGRESS_RESET_STATE.unlockTimer){
      clearTimeout(DBA_TOP_PROGRESS_RESET_STATE.unlockTimer);
      DBA_TOP_PROGRESS_RESET_STATE.unlockTimer = 0;
    }

    const now = Date.now();
    const unlockAt = loadTopProgressResetUnlockAt();
    if(!(unlockAt > now)){
      updateTopProgressResetButtons();
      return;
    }

    const delayMs = Math.max(0, unlockAt - now);
    DBA_TOP_PROGRESS_RESET_STATE.unlockTimer = window.setTimeout(() => {
      DBA_TOP_PROGRESS_RESET_STATE.unlockTimer = 0;
      if(!isTopProgressResetLocked(Date.now())){
        updateTopProgressResetButtons(true);
      }else{
        scheduleTopProgressResetUnlockCheck();
      }
    }, delayMs);
  }

  function updateTopProgressResetButtons(force){
    const locked = isTopProgressResetLocked(Date.now());
    if(!force && DBA_TOP_PROGRESS_RESET_STATE.lastDisabled === locked) return;
    DBA_TOP_PROGRESS_RESET_STATE.lastDisabled = locked;

    const buttons = [
      document.getElementById('dba-top-progress-reset-sync'),
      document.getElementById('dba-arena-top-progress-reset-sync')
    ];
    ...
    for(const btn of buttons){
      ...
      btn.title = locked
        ? '60秒後に再び使用できます。'
        : '保持している同期データを破棄して、同期を最初から取り直します。';
    }
  }

  function clearTopElapsedProgressSyncDataForReset(){
    try{ dbaStorageRemoveItem(LS_TOP_ELAPSED_PROGRESS_KEY); }catch(_e){}
    try{ resetMatchProgressAnchor(); }catch(_e){}
    try{ resetMatchSyncPlan(); }catch(_e){}

    try{
      DBA_TOP_PROGRESS_AUTOSYNC.doneAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.activeReason = '';
      DBA_TOP_PROGRESS_AUTOSYNC.lastManualAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.lastAutoAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.lastRejectedAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.lastResumeAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.hiddenAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.lastSyncingLabelAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.syncingLabelVisibleUntil = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.failCount = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.nextRetryAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncStartedAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncHardStopped = false;
    }catch(_e){}

    try{
      DBA_TOP_PROGRESS_RENDER_CACHE.termPct = null;
      DBA_TOP_PROGRESS_RENDER_CACHE.matchLabel = '';
      DBA_TOP_PROGRESS_RENDER_CACHE.matchTime = '';
      DBA_TOP_PROGRESS_RENDER_CACHE.matchPct = null;
      DBA_TOP_PROGRESS_RENDER_CACHE.termTitle = '';
      DBA_TOP_PROGRESS_RENDER_CACHE.syncDisabled = null;
      DBA_TOP_PROGRESS_RENDER_CACHE.syncText = '';
    }catch(_e){}
  }

  async function resetTopElapsedProgressAndResync(){
    if(isTopProgressResetLocked(Date.now())) return false;

    // 「60秒間のsetTimeout」ではなく、解除予定の絶対時刻を保存してから予約する。
    saveTopProgressResetUnlockAt(Date.now() + 60000);
    updateTopProgressResetButtons(true);
    scheduleTopProgressResetUnlockCheck();

    clearTopElapsedProgressSyncDataForReset();

    try{ renderTopProgress(); }catch(_e){}
    try{ renderArenaTermProgress(); }catch(_e){}

    if(DBA_TOP_PROGRESS_AUTOSYNC.inFlight) return false;

    try{
      return await fetchTopElapsedProgressFromTopPageOnce('manual');
    }catch(_e){
      return false;
    }finally{
      try{ renderTopProgress(); }catch(_e){}
  ...
  function ensureTopProgressContainer(targetParent){
    ...
    // 「Reset」ボタン：保持している同期データを破棄して同期を取り直す（RedBlue）
    const btnReset = document.createElement('button');
    btnReset.type = 'button';
    btnReset.id = 'dba-top-progress-reset-sync';
    btnReset.className = 'dba-btn-progress-mini';
    btnReset.textContent = 'Reset';
    btnReset.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      resetTopElapsedProgressAndResync().catch(()=>{});
    });
  ...
  function renderTopProgress(){
    ...
    if(btnSync){
      ...
      const syncingLabelVisible = shouldShowTopProgressSyncingLabel(currentNow);
      const syncText = syncingLabelVisible ? '同期中…' : '同期';
  ...
  // 方針：
  //  - 通常時はローカル時計で進める
  //  - ブラウザ起動直後などの初回自動同期は、試合時間を圧迫しないよう短時間で打ち切る
  //  - 初回自動同期は20秒程度を目標、30秒を上限として、粗い推定でも通常操作へ戻す
  //  - 通常の自動同期は「試合境界の前後」ではなく、各試合内の予定地点だけ行う
  ...
  const DBA_RB_LIVE_MAP_FAST_FETCH_TIMEOUT_MS = 3500;            // RB戦況更新用：軽量JSON取得の単発タイムアウト
  const DBA_RB_CELL_DETAIL_CACHE_MAX_ENTRIES = 3000;              // RB長押し戦況情報：セル詳細表示キャッシュの最大件数
  const DBA_RB_CELL_DETAIL_CACHE_MAX_AGE_MS = 10 * 60 * 1000;     // RB長押し戦況情報：セル詳細表示キャッシュの有効期間
  const DBA_TOP_PROGRESS_STARTUP_SYNC_TARGET_MS = 20 * 1000;      // 起動直後の自動同期は20秒程度で終える
  const DBA_TOP_PROGRESS_STARTUP_SYNC_HARD_LIMIT_MS = 30 * 1000;  // 起動直後の自動同期は長くても30秒で打ち切る
  const DBA_TOP_PROGRESS_AUTO_FETCH_TIMEOUT_MS = 12 * 1000;       // 自動同期fetchの単発タイムアウト
  const DBA_TOP_PROGRESS_MANUAL_FETCH_TIMEOUT_MS = 25 * 1000;     // 手動同期fetchの単発タイムアウト
  const DBA_TOP_PROGRESS_SYNCING_LABEL_THROTTLE_MS = 5 * 1000;    // 自動同期の「同期中…」表示は頻回点滅を避ける
  ...
  const DBA_TOP_PROGRESS_AUTOSYNC = {
    ...
    hiddenAt: 0,
    lastTickAt: 0,
    lastSyncingLabelAt: 0,
    syncingLabelVisibleUntil: 0,
    failCount: 0,
    nextRetryAt: 0,
    startupSyncStartedAt: 0,
    startupSyncFinishedAt: 0,
    startupSyncHardStopped: false
  };
  ...
  function isStartupTopProgressAutoSyncReason(reason){
    const rs = String(reason || '');
    if(rs === 'unsynced') return true;
    if(
      rs === 'start-switch' &&
      !(Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt || 0) > 0)
    ){
      return true;
    }
    return false;
  ...
  function ensureTopProgressStartupSyncBudgetStarted(now){
    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    if(!(Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncStartedAt || 0) > 0)){
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncStartedAt = currentNow;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt = 0;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncHardStopped = false;
    }
    return Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncStartedAt || currentNow);
  }

  function parseTermFromCurrentTeambattleHeader(){
    try{
      const header = document.querySelector('header');
      const text = sanitizeText(header ? header.textContent : '');
      const m = text.match(/第\s*\d+\s*期/);
      return m ? sanitizeText(m[0]) : '';
    }catch(_e){
      return '';
    }
  }

  function seedApproxTopProgressFromLocalClock(reason){
    try{
      if(loadTopElapsedProgress()) return false;

      const d = new Date();
      const elapsedSec = (
        (d.getMinutes() * 60) +
        d.getSeconds() +
        Math.floor(d.getMilliseconds() / 1000)
      ) % 3600;
      const pct = Math.max(0, Math.min(100, (elapsedSec / 3600) * 100));
      const term =
        parseTermFromCurrentTeambattleHeader() ||
        parseElapsedTermFromTopDoc(document) ||
        '';

      saveTopElapsedProgress(pct, term);
      try{
        setMatchProgressAnchorFromElapsedSec(elapsedSec, term);
      }catch(_e){}
  ...
  function shouldAllowTopProgressAutoSyncAttempt(reason, now){
    const rs = String(reason || '');
    if(rs === 'manual') return true;
    if(!isStartupTopProgressAutoSyncReason(rs)) return true;

    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();

    if(Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt || 0) > 0){
      return false;
    }

    const startedAt = ensureTopProgressStartupSyncBudgetStarted(currentNow);
    const elapsedMs = Math.max(0, currentNow - startedAt);

    if(
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncHardStopped ||
      elapsedMs >= DBA_TOP_PROGRESS_STARTUP_SYNC_HARD_LIMIT_MS
    ){
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncHardStopped = true;
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt = currentNow;
      seedApproxTopProgressFromLocalClock(`startup-hard-stop:${rs}`);
      try{ renderTopProgress(); }catch(_e){}
      return false;
    }
  ...
  function beginTopProgressSyncingLabel(reason, now){
    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    const rs = String(reason || '');

    DBA_TOP_PROGRESS_AUTOSYNC.activeReason = rs;

    // 手動同期はユーザー操作への反応なので必ず表示する
    if(rs === 'manual'){
      DBA_TOP_PROGRESS_AUTOSYNC.lastSyncingLabelAt = currentNow;
      DBA_TOP_PROGRESS_AUTOSYNC.syncingLabelVisibleUntil = currentNow + DBA_TOP_PROGRESS_MANUAL_FETCH_TIMEOUT_MS;
      return true;
    }

    // 自動同期は短時間に連発した場合の点滅を避け、5秒に1回程度へ間引く
    const lastShownAt = Number(DBA_TOP_PROGRESS_AUTOSYNC.lastSyncingLabelAt || 0);
    if(!(lastShownAt > 0) || (currentNow - lastShownAt) >= DBA_TOP_PROGRESS_SYNCING_LABEL_THROTTLE_MS){
      DBA_TOP_PROGRESS_AUTOSYNC.lastSyncingLabelAt = currentNow;
      DBA_TOP_PROGRESS_AUTOSYNC.syncingLabelVisibleUntil = currentNow + DBA_TOP_PROGRESS_SYNCING_LABEL_THROTTLE_MS;
      return true;
    }
  ...
  function shouldShowTopProgressSyncingLabel(now){
    if(!DBA_TOP_PROGRESS_AUTOSYNC.inFlight) return false;

    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    const rs = String(DBA_TOP_PROGRESS_AUTOSYNC.activeReason || '');

    if(rs === 'manual') return true;
    return currentNow <= Number(DBA_TOP_PROGRESS_AUTOSYNC.syncingLabelVisibleUntil || 0);
  }

  function finishTopProgressStartupSyncAttempt(reason, accepted){
    const rs = String(reason || '');
    if(!isStartupTopProgressAutoSyncReason(rs)) return;

    const currentNow = Date.now();
    const startedAt = Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncStartedAt || 0);
    if(!(startedAt > 0)) return;
    if(Number(DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt || 0) > 0) return;
    ...
    if(
      ...
      rs === 'unsynced' ||
      rs === 'start-switch' ||
      elapsedMs >= DBA_TOP_PROGRESS_STARTUP_SYNC_TARGET_MS ||
      elapsedMs >= DBA_TOP_PROGRESS_STARTUP_SYNC_HARD_LIMIT_MS
    ){
      DBA_TOP_PROGRESS_AUTOSYNC.startupSyncFinishedAt = currentNow;
      if(!loadTopElapsedProgress()){
        seedApproxTopProgressFromLocalClock(`startup-finished-approx:${rs}`);
      }
      if(elapsedMs >= DBA_TOP_PROGRESS_STARTUP_SYNC_HARD_LIMIT_MS){
        DBA_TOP_PROGRESS_AUTOSYNC.startupSyncHardStopped = true;
        seedApproxTopProgressFromLocalClock(`startup-finished-hard:${rs}`);
      }
    }
  }
  ...
  function markMatchSyncAttemptConsumed(reason, now){
    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    DBA_MATCH_SYNC_PLAN.lastSyncReason = String(reason || '');
    DBA_MATCH_SYNC_PLAN.pendingResumeEventAt = 0;
  ...
  async function fetchTopElapsedProgressFromTopPageOnce(reason){
    ...
    const now = Date.now();
    const isManual = (String(reason || '') === 'manual');
    if(!shouldAllowTopProgressAutoSyncAttempt(reason, now)){
      return false;
    }
    ...
    beginTopProgressSyncingLabel(reason, now);
    let timeoutId = 0;
    let accepted = false;
    try{
      // メタ表示を更新（見た目だけ）
      ...
      const timeoutMs = isManual
        ? DBA_TOP_PROGRESS_MANUAL_FETCH_TIMEOUT_MS
        : DBA_TOP_PROGRESS_AUTO_FETCH_TIMEOUT_MS;
      ...
      if(!judged.accept){
        ...
        markMatchSyncAttemptConsumed(reason, Date.now());
        try{ renderTopProgress(); }catch(_e){}
        return false;
      }
      ...
      DBA_TOP_PROGRESS_AUTOSYNC.activeReason = '';
      if(!isManual){
        finishTopProgressStartupSyncAttempt(reason, accepted);
      }
      try{ renderTopProgress(); }catch(_e){}
  ...
  function parseDonguriNameFromTopDoc(doc){
    if(!doc) return '';

    const candidates = Array.from(doc.querySelectorAll('.stats.header div[style]'));
    for(const el of candidates){
      const style = String(el.getAttribute('style') || '');
      if(!/font-size\s*:\s*2em/i.test(style)) continue;
  ...
  function captureDonguriNameFromTopDoc(doc){
    const name = parseDonguriNameFromTopDoc(doc);
    if(name) saveStoredDonguriName(name);
    return name;
  }

  async function fetchDonguriNameFromTopPage(){
    try{
      const res = await fetch(makeTopPageUrl(), {
        method: 'GET',
        credentials: 'include',
      ...
      return captureDonguriNameFromTopDoc(doc);
    }catch(_e){
      return '';
    }
  }

  async function ensureStoredDonguriName(){
    const stored = loadStoredDonguriName();
    if(stored) return stored;
    return await fetchDonguriNameFromTopPage();
  }

  async function resolveArenaLogsSelfFilterName(shouldEnable){
    const stored = loadStoredDonguriName();
    if(stored) return stored;
  ...
  function captureTopPageElapsedProgress(){
    try{
      const capturedName = captureDonguriNameFromTopDoc(document);
      const pct = parseElapsedProgressPctFromTopDoc(document);
      if(pct == null) return !!capturedName;
  ...
  function parseArenaTermFromHeader(doc){
    if(!doc) return '';
    const header = doc.querySelector('header');
    const h4 = header ? header.querySelector('h4') : null;
    const text = sanitizeText(h4 ? h4.textContent : '');
    const m = text.match(/第\s*\d+\s*期/);
    return m ? sanitizeText(m[0]) : '';
  }

  function ensureArenaJapanImageRelocated(){
    const header = document.querySelector('header');
    if(!header || !header.parentNode) return false;

    let host = document.getElementById('img-donguri-japan');
    if(!host){
      host = document.createElement('div');
      host.id = 'img-donguri-japan';
    }else{
      host.textContent = '';
    }

    const srcImg = header.querySelector('img[src="/donguri-japan.png"], img[src$="/donguri-japan.png"]');
    if(srcImg){
      host.appendChild(srcImg.cloneNode(true));
    }

    const next = header.nextSibling;
    if(host.parentNode !== header.parentNode){
      header.parentNode.insertBefore(host, next);
    }else if(host.previousSibling !== header){
      header.parentNode.insertBefore(host, next);
    }
    return true;
  }

  function syncArenaUtilityHeight(){
    try{
      const bar = document.getElementById('dba-arena-utility-section');
      if(!bar) return;
      const h = Math.ceil(bar.getBoundingClientRect().height || 0);
      document.documentElement.style.setProperty('--dba-arena-util-height', `${Math.max(0, h)}px`);
    }catch(_e){}
  }

  function renderArenaTermProgress(){
    const inner = document.getElementById('dba-top-progress-pop-inner');
    if(!inner) return;
    ...
    const data = loadTopElapsedProgress();
    const title = document.getElementById('dba-arena-utility-title');
    if(title){
      const headerTerm = title.dataset.headerTerm || '';
      const term = (data && data.term) ? data.term : headerTerm;
      title.textContent = `どんぐりアリーナ　${term || '第 ? 期'}`;
    }
  }

    updateTopProgressResetButtons();

  let DBA_ARENA_TERM_PROGRESS_TID = 0;
  function startArenaTermProgressTicker(){
    if(DBA_ARENA_TERM_PROGRESS_TID) return;
    renderArenaTermProgress();
    DBA_ARENA_TERM_PROGRESS_TID = window.setInterval(() => {
      try{ renderArenaTermProgress(); }catch(_e){}
    }, 1000);
  }

  function buildArenaUtilitySection(){
    const headerTerm = parseArenaTermFromHeader(document);

    const wrap = document.createElement('div');
    wrap.id = 'dba-arena-utility-section';

    const title = document.createElement('div');
    title.id = 'dba-arena-utility-title';
    title.className = 'dba-arena-util__title';
    title.dataset.headerTerm = headerTerm;
    title.textContent = `どんぐりアリーナ　${headerTerm || '第 ? 期'}`;

    const row2 = document.createElement('div');
    row2.className = 'dba-arena-util__row';

    const termBar = document.createElement('div');
    termBar.className = 'dba-arena-util__termBar';

    const termInner = document.createElement('div');
    // 既存のポップアップ内「期」プログレス表示と同じ id を arena 側の固定バーとして流用する。
    termInner.id = 'dba-top-progress-pop-inner';
    termInner.style.width = '0%';
    termInner.textContent = '0%';
    termBar.appendChild(termInner);

    // 「同期」ボタン（アリーナ）
    const btnSync = document.createElement('button');
    btnSync.type = 'button';
    btnSync.id = 'dba-arena-top-progress-sync-now';
    btnSync.className = 'dba-btn-progress-mini';
    btnSync.textContent = '同期';
    btnSync.addEventListener('click', async (e) => {
      ...
      try{
        await fetchTopElapsedProgressFromTopPageOnce('manual');
      }catch(_e){}
      finally{
    ...
    // 「Reset」ボタン：保持している同期データを破棄して同期を取り直す（アリーナ）
    const btnReset = document.createElement('button');
    btnReset.type = 'button';
    btnReset.id = 'dba-arena-top-progress-reset-sync';
    btnReset.className = 'dba-btn-progress-mini';
    btnReset.textContent = 'Reset';
    btnReset.addEventListener('click', async (e) => {
      e.preventDefault();
      e.stopPropagation();
      await resetTopElapsedProgressAndResync();
      renderArenaTermProgress();
    });

    row2.appendChild(termBar);
    row2.appendChild(btnSync);
  ...
  function initArenaPageUtility(){
    if(document.getElementById('dba-arena-utility-section')) return false;

    try{ captureTopPageElapsedProgress(); }catch(_e){}

    document.documentElement.classList.add('dba-has-arena-util');

    const header = document.querySelector('header');
    const parent = header && header.parentNode ? header.parentNode : (document.body || document.documentElement);
    const util = buildArenaUtilitySection();
    if(header && header.parentNode){
      header.parentNode.insertBefore(util, header);
      header.style.display = 'none';
    }else{
      parent.insertBefore(util, parent.firstChild);
    }

    ensureArenaJapanImageRelocated();
    startArenaTermProgressTicker();
    syncArenaUtilityHeight();
    window.addEventListener('resize', syncArenaUtilityHeight, { passive:true });
    window.addEventListener('orientationchange', syncArenaUtilityHeight, { passive:true });
    setTimeout(syncArenaUtilityHeight, 50);
    setTimeout(syncArenaUtilityHeight, 300);
    return true;
  }

  // =========================
  // どんぐりアリーナ：リーダーチャレンジ結果モーダル
  //  - /arena の「リーダーに挑む」submitをページ遷移させず fetch で取得
  //  - サーバーから返ったテキストを「アリーナリザルト」モーダルへ表示
  //  - 表示中テキストを .txt として保存
  // =========================
  function buildArenaResultModal(){
    if(document.getElementById('dba-m-arena-result')) return;

    const dlg = document.createElement('dialog');
    dlg.id = 'dba-m-arena-result';
    dlg.className = 'dba-m-std';
    dlg.dataset.dbaArenaResultText = '';
    ...
    title.textContent = 'アリーナリザルト';

    const btnX = document.createElement('button');
    btnX.type = 'button';
    btnX.className = 'dba-btn-x';
    btnX.textContent = '×';
    btnX.setAttribute('aria-label', '閉じる');
    ...
    box.id = 'dba-arena-result-box';
    box.className = 'dba-arena-result-text';
    box.textContent = '';
    mid.appendChild(box);
    ...
    btnSave.type = 'button';
    btnSave.id = 'dba-arena-result-save-txt';
    btnSave.className = 'dba-btn-apply';
    btnSave.textContent = 'txt形式で保存';
    ...
    btnSave.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      downloadArenaResultTextFile(dlg.dataset.dbaArenaResultText || '');
    });
  ...
  function openArenaResultModalWithText(text){
    buildArenaResultModal();

    const dlg = document.getElementById('dba-m-arena-result');
    const box = document.getElementById('dba-arena-result-box');
    if(!dlg || !box) return;

    const resultText = String(text ?? '');
    dlg.dataset.dbaArenaResultText = resultText;
    box.textContent = resultText || '結果を表示できませんでした。';

    try{ dlg.showModal(); }catch(_e){ dlg.setAttribute('open', ''); }

    const mid = dlg.querySelector('.dba-modal__mid');
    if(mid instanceof HTMLElement){
      mid.scrollTop = 0;
    }
  }

  function makeArenaResultTextFileName(){
    const d = new Date();
    const pad = (n) => String(n).padStart(2, '0');
    const stamp =
      `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}_` +
      `${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;
    return `donguri_arena_result_${stamp}.txt`;
  }

  function downloadArenaResultTextFile(text){
    ...
    try{
      ...
      a.href = url;
      a.download = makeArenaResultTextFileName();
  ...
  function isArenaChallengeForm(form, submitter){
    if(!(form instanceof HTMLFormElement)) return false;

    const action = String(form.getAttribute('action') || '');
    const actionUrl = new URL(action || location.href, location.href);
    if(actionUrl.pathname !== '/challenge') return false;

    const submitText = sanitizeText(
      submitter
        ? (submitter.textContent || submitter.value || '')
        : ''
    );
    if(submitText && !submitText.includes('リーダーに挑む')) return false;

    // /arena のリーダー列フォームは region を送る。
    // 万一別用途の /challenge form が増えた場合に誤捕捉しにくくする。
    const regionInput = form.querySelector('input[name="region"]');
    return !!regionInput;
  }

  function normalizeArenaResultTextFromResponse(bodyText, contentType){
    ...
    if(ct.includes('text/html')){
      try{
        ...
        const text = doc.body ? (doc.body.innerText || doc.body.textContent || '') : raw;
  ...
  async function fetchArenaChallengeResult(form, submitter){
    ...
    openArenaResultModalWithText('アリーナリザルトを取得中…');
    ...
    const resultText = normalizeArenaResultTextFromResponse(
      bodyText,
      res.headers.get('content-type') || ''
    );

    if(!res.ok){
      throw new Error(`HTTP ${res.status}: ${resultText || res.statusText || 'Request failed'}`);
    }

    openArenaResultModalWithText(resultText || '結果を表示できませんでした。');
  }

  function installArenaChallengeResultInterception(){
    if(document.documentElement.dataset.dbaArenaChallengeResultInstalled === '1') return;
    document.documentElement.dataset.dbaArenaChallengeResultInstalled = '1';
    ...
    document.addEventListener('submit', (e) => {
      ...
      if(!isArenaChallengeForm(form, submitter)) return;

      e.preventDefault();
      e.stopImmediatePropagation();

      if(form.dataset.dbaArenaChallengeInFlight === '1') return;
      form.dataset.dbaArenaChallengeInFlight = '1';

      fetchArenaChallengeResult(form, submitter).catch((err) => {
        const msg = String(err && err.message ? err.message : err || '取得に失敗しました。');
        openArenaResultModalWithText(`アリーナリザルトの取得に失敗しました。\n\n${msg}`);
      }).finally(() => {
        form.dataset.dbaArenaChallengeInFlight = '0';
      });
    }, true);
  }

  function buildArenaCenterModal(){
    if(document.getElementById('dba-m-arena-center')) return;

    const dlg = document.createElement('dialog');
    dlg.id = 'dba-m-arena-center';
    dlg.className = 'dba-m-std';
    ...
    const box = document.createElement('div');
    box.id = 'dba-arena-center-box';
    box.className = 'dba-arena-center-content';
    box.textContent = '';
    mid.appendChild(box);
  ...
  function openArenaCenterModalWithNode(nodeOrText){
    buildArenaCenterModal();

    const dlg = document.getElementById('dba-m-arena-center');
    const box = document.getElementById('dba-arena-center-box');
    if(!dlg || !box) return;
    ...
    if(nodeOrText instanceof Node){
      ...
      pre.className = 'dba-arena-center-plain';
      pre.textContent = String(nodeOrText ?? '') || 'アリーナセンターを表示できませんでした。';
      box.appendChild(pre);
    }
  ...
  function buildArenaCenterResultDialog(){
    if(document.getElementById('dba-m-arena-center-result')) return;

    const dlg = document.createElement('dialog');
    dlg.id = 'dba-m-arena-center-result';
    ...
    mid.id = 'dba-arena-center-result-text';
    mid.textContent = '';
  ...
  function openArenaCenterResultDialogWithText(text){
    buildArenaCenterResultDialog();

    const dlg = document.getElementById('dba-m-arena-center-result');
    const mid = document.getElementById('dba-arena-center-result-text');
    if(!dlg || !mid) return;

    mid.textContent = String(text ?? '').replace(/\r\n?/g, '\n').trim() || '結果を表示できませんでした。';

    try{ dlg.showModal(); }catch(_e){ dlg.setAttribute('open', ''); }
    try{ mid.scrollTop = 0; }catch(_e){}
  }

  function isArenaCenterEntryForm(form, submitter){
    if(!(form instanceof HTMLFormElement)) return false;

    const action = String(form.getAttribute('action') || '');
    const actionUrl = new URL(action || location.href, location.href);
    if(actionUrl.pathname !== '/reinforce' && actionUrl.pathname !== '/siege') return false;
    ...
    if(
      submitText &&
      !submitText.includes('アリーナを強化する') &&
      !submitText.includes('アリーナを弱体化する')
    ){
      return false;
    }
  ...
  function isArenaCenterActionForm(form, submitter){
    if(!(form instanceof HTMLFormElement)) return false;

    const action = String(form.getAttribute('action') || '');
    const actionUrl = new URL(action || location.href, location.href);
    if(actionUrl.pathname !== '/arenavol/') return false;

    const actionInput = form.querySelector('input[name="action"]');
    const actionValue = String(actionInput ? actionInput.value : '');
    if(actionValue !== 'ReinforceArena' && actionValue !== 'SiegeArena') return false;
  ...
  function dbaMakeAbsoluteUrlMaybe(value){
  ...
  function normalizeArenaCenterContentFromResponse(bodyText){
    const raw = String(bodyText ?? '');
    if(!raw) return 'アリーナセンターを表示できませんでした。';
    ...
    if(!doc) return raw.replace(/\r\n?/g, '\n').trim();

    const wrap = document.createElement('div');
    wrap.className = 'dba-arena-center-content';

    const srcHeader = doc.querySelector('header');
    if(srcHeader){
      const header = srcHeader.cloneNode(true);
      if(header instanceof HTMLElement){
        const h1s = Array.from(header.querySelectorAll('h1'));
        for(const h1 of h1s){
          h1.remove();
        }
        ...
        for(const img of imgs){
          ...
          if(src){
            img.setAttribute('src', dbaMakeAbsoluteUrlMaybe(src));
          }
        }
    ...
    const form =
      doc.querySelector('form[action="/arenavol/"]') ||
      Array.from(doc.querySelectorAll('form')).find((f) => {
        try{
          const u = new URL(String(f.getAttribute('action') || ''), location.href);
          return u.pathname === '/arenavol/';
        }catch(_e){
          return false;
        }
      });
    ...
    if(form){
      ...
      if(clonedForm instanceof HTMLFormElement){
        const action = clonedForm.getAttribute('action');
        if(action){
          clonedForm.setAttribute('action', dbaMakeAbsoluteUrlMaybe(action));
        }
        wrap.appendChild(clonedForm);
      }
    }
    ...
    const text = doc.body ? (doc.body.innerText || doc.body.textContent || '') : raw;
    return String(text || raw).replace(/\r\n?/g, '\n').trim();
  }

  async function fetchArenaCenterPage(form, submitter){
    const method = (form.method || 'GET').toUpperCase();
    const action = form.getAttribute('action') || location.href;
    const url = new URL(action, location.href);
    ...
    openArenaCenterModalWithNode('アリーナセンターを取得中…');
    ...
    const res = await fetch(fetchUrl, init);
    const bodyText = await res.text();
    const nodeOrText = normalizeArenaCenterContentFromResponse(bodyText);

    if(!res.ok){
      const msg = normalizeArenaResultTextFromResponse(
        bodyText,
        res.headers.get('content-type') || ''
      );
      throw new Error(`HTTP ${res.status}: ${msg || res.statusText || 'Request failed'}`);
    }

    openArenaCenterModalWithNode(nodeOrText);
  }

  async function fetchArenaCenterActionResult(form, submitter){
    const method = (form.method || 'GET').toUpperCase();
    const action = form.getAttribute('action') || location.href;
    const url = new URL(action, location.href);
    ...
    openArenaCenterResultDialogWithText('アリーナセンターへ送信中…');
    ...
    openArenaCenterResultDialogWithText(resultText || '結果を表示できませんでした。');
  }

  function installArenaCenterInterception(){
    if(document.documentElement.dataset.dbaArenaCenterInstalled === '1') return;
    document.documentElement.dataset.dbaArenaCenterInstalled = '1';
    ...
    document.addEventListener('submit', (e) => {
      ...
      if(isArenaCenterActionForm(form, submitter)){
        e.preventDefault();
        e.stopImmediatePropagation();

        if(form.dataset.dbaArenaCenterActionInFlight === '1') return;
        form.dataset.dbaArenaCenterActionInFlight = '1';

        fetchArenaCenterActionResult(form, submitter).catch((err) => {
          const msg = String(err && err.message ? err.message : err || '送信に失敗しました。');
          openArenaCenterResultDialogWithText(`アリーナセンターへの送信に失敗しました。\n\n${msg}`);
        }).finally(() => {
          form.dataset.dbaArenaCenterActionInFlight = '0';
        });
        return;
      }

      if(isArenaCenterEntryForm(form, submitter)){
        e.preventDefault();
        e.stopImmediatePropagation();

        if(form.dataset.dbaArenaCenterInFlight === '1') return;
        form.dataset.dbaArenaCenterInFlight = '1';

        fetchArenaCenterPage(form, submitter).catch((err) => {
          const msg = String(err && err.message ? err.message : err || '取得に失敗しました。');
          openArenaCenterModalWithNode(`アリーナセンターの取得に失敗しました。\n\n${msg}`);
        }).finally(() => {
          form.dataset.dbaArenaCenterInFlight = '0';
        });
      }
    }, true);
  }
  ...
  const DEFAULT_SETTINGS = {
    ...
    // レッドvsブルー：Reg.表示・境界線・首都王冠・誰もいない
    rbLayer: {
  ...
  function loadBattleResultTail2Enabled(){
    try{
      const raw = dbaStorageGetItem(LS_BR_TAIL2_KEY);
      // デフォルトOFF
      if(raw == null) return false;
      return raw === '1';
    }catch(_e){
  ...
  function loadBattleResultAlign(){
    try{
      const raw = dbaStorageGetItem(LS_BR_ALIGN_KEY);
  ...
  function saveBattleResultAlign(v){
    ...
    try{ dbaStorageSetItem(LS_BR_ALIGN_KEY, x); }catch(_e){}
  }

  function loadBattleResultPassThrough(){
    try{
      const raw = dbaStorageGetItem(LS_BR_PASS_KEY);
      if(raw == null) return false;
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveBattleResultPassThrough(on){
    try{ dbaStorageSetItem(LS_BR_PASS_KEY, on ? '1' : '0'); }catch(_e){}
  }

  function sanitizeBattleResultAutoCloseSec(v){
    const n = Number(v);
    if(!Number.isFinite(n)) return 5.0;
    const rounded = Math.round(n * 10) / 10;
    return Math.max(1.0, Math.min(86400.0, rounded));
  }

  function loadBattleResultAutoCloseEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_BR_AUTOCLOSE_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveBattleResultAutoCloseEnabled(on){
    try{ dbaStorageSetItem(LS_BR_AUTOCLOSE_KEY, on ? '1' : '0'); }catch(_e){}
  }

  function loadBattleResultAutoCloseSec(){
    try{
      const raw = dbaStorageGetItem(LS_BR_AUTOCLOSE_SEC_KEY);
      if(raw == null) return 5.0; // デフォルト 5.0秒
      return sanitizeBattleResultAutoCloseSec(raw);
    }catch(_e){
      return 5.0;
    }
  }

  function saveBattleResultAutoCloseSec(sec){
    const n = sanitizeBattleResultAutoCloseSec(sec);
    try{ dbaStorageSetItem(LS_BR_AUTOCLOSE_SEC_KEY, String(n)); }catch(_e){}
  }

  function cancelBattleResultAutoCloseTimer(){
    if(DBA_BATTLE_RESULT_AUTOCLOSE.timer){
      clearTimeout(DBA_BATTLE_RESULT_AUTOCLOSE.timer);
      DBA_BATTLE_RESULT_AUTOCLOSE.timer = 0;
    }
  }

  function closeBattleResultDialogDirect(){
  ...
  function shouldStartBattleResultAutoClose(text){
    const s = sanitizeText(text || '');
    if(!s) return false;
    if(/取得中/.test(s)) return false;
    return true;
  }

  function scheduleBattleResultAutoClose(){
    cancelBattleResultAutoCloseTimer();
    if(!loadBattleResultAutoCloseEnabled()) return;

    const sec = loadBattleResultAutoCloseSec();
    const delayMs = Math.max(1000, Math.round(sec * 1000));

    DBA_BATTLE_RESULT_AUTOCLOSE.timer = setTimeout(() => {
      DBA_BATTLE_RESULT_AUTOCLOSE.timer = 0;
      ...
      if(dlg && (dlg.open || dlg.hasAttribute('open'))){
        closeBattleResultDialogDirect();
      }
    }, delayMs);
  }

  function rearmBattleResultAutoCloseIfOpen(){
    const floatRoot = document.getElementById('dba-br-float');
    if(floatRoot && floatRoot.dataset.open === '1'){
      const full = String(floatRoot.dataset.dbaBattleFullText || '');
      if(shouldStartBattleResultAutoClose(full)){
    ...
    const dlg = document.getElementById('dba-m-battle-result');
    if(dlg && (dlg.open || dlg.hasAttribute('open'))){
      const full = String(dlg.dataset.dbaBattleFullText || '');
      if(shouldStartBattleResultAutoClose(full)){
        scheduleBattleResultAutoClose();
      }else{
        cancelBattleResultAutoCloseTimer();
  ...
  function loadBattleResultHideClose(){
    try{
      const raw = dbaStorageGetItem(LS_BR_HIDE_CLOSE_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveBattleResultHideClose(on){
    try{ dbaStorageSetItem(LS_BR_HIDE_CLOSE_KEY, on ? '1' : '0'); }catch(_e){}
  }

  function loadBattleResultHideOption(){
    try{
      const raw = dbaStorageGetItem(LS_BR_HIDE_OPT_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveBattleResultHideOption(on){
    try{ dbaStorageSetItem(LS_BR_HIDE_OPT_KEY, on ? '1' : '0'); }catch(_e){}
  }

  function loadBattleResultTopOffsetPx(){
    try{
      const raw = dbaStorageGetItem(LS_BR_TOP_KEY);
      // 未設定は 0px（今回要望）
  ...
  function saveBattleResultTopOffsetPx(px){
    ...
    try{ dbaStorageSetItem(LS_BR_TOP_KEY, String(n)); }catch(_e){}
  }

  // 戦闘結果ウインドウの width のデフォルト値：300px
  function loadBattleResultWidthPx(){
    try{
      const raw = dbaStorageGetItem(LS_BR_WIDTH_KEY);
  ...
  function saveBattleResultWidthPx(px){
    ...
    try{ dbaStorageSetItem(LS_BR_WIDTH_KEY, String(n)); }catch(_e){}
  }

  function loadRapidAttackEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_RAPID_KEY);
      // デフォルトOFF
      if(raw == null) return false;
      return raw === '1';
    }catch(_e){
  ...
  function saveRapidAttackEnabled(on){
    try{
      dbaStorageSetItem(LS_RAPID_KEY, on ? '1' : '0');
    }catch(_e){}
  }

  function loadAutoEquipEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_AUTO_EQUIP_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveAutoEquipEnabled(on){
    try{ dbaStorageSetItem(LS_AUTO_EQUIP_KEY, on ? '1' : '0'); }catch(_e){}
  }
  ...
  function migrateLegacyAutoEquipToRosterIfNeeded(){
    ...
    let last = '';
    try{
      const raw = dbaStorageGetItem(LS_AUTO_EQUIP_CAND_KEY);
      if(raw){
        const obj = JSON.parse(raw);
    ...
    try{
      const raw2 = dbaStorageGetItem(LS_AUTO_EQUIP_LAST_KEY);
  ...
  function loadAutoEquipNotifyAutoCloseEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_AUTO_EQUIP_NOTIFY_AUTOCLOSE_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveAutoEquipNotifyAutoCloseEnabled(on){
    try{ dbaStorageSetItem(LS_AUTO_EQUIP_NOTIFY_AUTOCLOSE_KEY, on ? '1' : '0'); }catch(_e){}
  }
  ...
  function loadAutoEquipNotifyAutoCloseSec(){
    try{
      const raw = dbaStorageGetItem(LS_AUTO_EQUIP_NOTIFY_AUTOCLOSE_SEC_KEY);
  ...
  function saveAutoEquipNotifyAutoCloseSec(sec){
    ...
    try{ dbaStorageSetItem(LS_AUTO_EQUIP_NOTIFY_AUTOCLOSE_SEC_KEY, String(n)); }catch(_e){}
  }

  function loadAutoEquipPreferTopEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_AUTO_EQUIP_PREFER_TOP_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveAutoEquipPreferTopEnabled(on){
    try{ dbaStorageSetItem(LS_AUTO_EQUIP_PREFER_TOP_KEY, on ? '1' : '0'); }catch(_e){}
  }

  function loadCurrentPresetName(){
    try{
      const raw = dbaStorageGetItem(LS_CURRENT_PRESET_NAME_KEY);
  ...
  function saveCurrentPresetName(name){
    try{
      dbaStorageSetItem(LS_CURRENT_PRESET_NAME_KEY, String(name || ''));
    }catch(_e){}
  }

  function loadRosterScrollRememberEnabled(){
    try{
      const raw = dbaStorageGetItem(LS_ROSTER_SCROLL_REMEMBER_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveRosterScrollRememberEnabled(on){
    try{
      dbaStorageSetItem(LS_ROSTER_SCROLL_REMEMBER_KEY, on ? '1' : '0');
    }catch(_e){}
  }

  function loadRosterSavedScrollTop(){
    try{
      const raw = dbaStorageGetItem(LS_ROSTER_SCROLL_TOP_KEY);
      if(raw == null) return 0;
      const n = Number.parseInt(raw, 10);
      if(!Number.isFinite(n)) return 0;
      return Math.max(0, n);
  ...
  function saveRosterSavedScrollTop(v){
    try{
      ...
      dbaStorageSetItem(LS_ROSTER_SCROLL_TOP_KEY, String(Number.isFinite(n) ? Math.max(0, n) : 0));
    }catch(_e){}
  }

  function clearRosterSavedScrollTop(){
    try{
      dbaStorageRemoveItem(LS_ROSTER_SCROLL_TOP_KEY);
    }catch(_e){}
  }

  function loadRosterPresetClickAction(){
    try{
      const raw = dbaStorageGetItem(LS_ROSTER_PRESET_CLICK_ACTION_KEY);
  ...
  function saveRosterPresetClickAction(modeValue){
    ...
    try{
      dbaStorageSetItem(LS_ROSTER_PRESET_CLICK_ACTION_KEY, v);
    }catch(_e){}
  }

  function loadRosterHideMenuRow2Enabled(){
    try{
      const raw = dbaStorageGetItem(LS_ROSTER_HIDE_MENU_ROW2_KEY);
      if(raw == null) return false; // デフォルトOFF
      return raw === '1';
    }catch(_e){
      return false;
  ...
  function saveRosterHideMenuRow2Enabled(on){
    try{
      dbaStorageSetItem(LS_ROSTER_HIDE_MENU_ROW2_KEY, on ? '1' : '0');
    }catch(_e){}
  }
  ...
  function openBattleResultModalWithNodeNonBlocking(nodeOrText, titleText){
    // v9.0.8.1:
    // 以前はRBの並行処理中に戦闘結果を強制的に透過フロート表示していたが、
    // これにより「戦闘結果：オプション」の
    // 『「Close」「オプション」ボタン以外はクリック判定を透過させる。』
    // がOFFでも透過してしまっていた。
    // 以後は全モード共通で、透過するかどうかは保存済みオプション
    // loadBattleResultPassThrough() のみで判定する。
  ...
  function disposeCurrentRbBattlemapInstance(reason){
    void reason;
    if(mode !== 'rb') return false;
    ...
    try{
      if(typeof api.dispose === 'function'){
        disposed = !!api.dispose();
      }
    }catch(_e){}

    try{
      if(window.__DBA_RB_API === api){
        delete window.__DBA_RB_API;
      }
    }catch(_e){
      try{ window.__DBA_RB_API = null; }catch(_e2){}
    }

    return disposed;
  }

  function shouldScheduleTeamChallengeAutoSync(){
    const s = loadSettings();
    return !!s?.postBattle?.autoDiffSync;
  }

  function scheduleTeamChallengeAutoSyncFromResultText(resultText, priorityTargets){
    if(!shouldScheduleTeamChallengeAutoSync()) return false;

    scheduleAutoBattleInfoSyncAfterBattleResult(priorityTargets).catch(()=>{});

    // 「戦闘ログ」本文を検出した場合も、ここで追加の差分同期を積まない。
    // autoSyncMode=diff では scheduleAutoBattleInfoSyncAfterBattleResult() が差分同期を担当し、
    // autoSyncMode=map ではマップのみ更新を担当するため、二重fetch/二重描画を避ける。
    void resultText;
    return true;
  }
  ...
  function sanitizePostTeamChallengeSyncKind(kind){
    const s = String(kind || '').trim().toLowerCase();
    return (s === 'diff') ? 'diff' : 'map';
  }

  function normalizePostTeamChallengeTarget(target){
    if(!target || typeof target !== 'object') return null;
    const row = Number(target.row ?? target.r);
    const col = Number(target.col ?? target.c);
    if(!Number.isFinite(row) || !Number.isFinite(col)) return null;
    if(row < 0 || col < 0) return null;
    return {
      row: Math.floor(row),
      col: Math.floor(col),
      source: String(target.source || 'teamchallenge'),
      at: Number.isFinite(Number(target.at)) ? Number(target.at) : Date.now()
    };
  }

  function normalizePostTeamChallengeTargets(targets){
    const src = Array.isArray(targets) ? targets : [targets];
    const out = [];
    const seen = new Set();

    for(const item of src){
      const t = normalizePostTeamChallengeTarget(item);
      if(!t) continue;
      const key = `${t.row},${t.col}`;
      if(seen.has(key)) continue;
      seen.add(key);
      out.push(t);
      if(out.length >= DBA_POST_TEAMCHALLENGE_PRIORITY_LIMIT) break;
    }

    return out;
  }

  function rememberLatestPostTeamChallengeTarget(row, col, source){
    const target = normalizePostTeamChallengeTarget({
      row,
      col,
      source: source || 'teamchallenge',
      at: Date.now()
    });
    ...
    // 古いものから並ぶ配列として保持し、同じセルを再クリックした場合は最新扱いで末尾へ移動する。
    list.push(target);
    while(list.length > DBA_POST_TEAMCHALLENGE_PRIORITY_LIMIT){
      list.shift();
    }

    DBA_POST_TEAMCHALLENGE_SYNC.latestTargets = list;
    DBA_POST_TEAMCHALLENGE_SYNC.seq += 1;
    return getLatestPostTeamChallengeTargets();
  }

  function getLatestPostTeamChallengeTargets(){
    return normalizePostTeamChallengeTargets(DBA_POST_TEAMCHALLENGE_SYNC.latestTargets);
  }

  function resolvePostTeamChallengePriorityTargets(targets){
    const normalized = normalizePostTeamChallengeTargets(targets);
    return normalized.length > 0 ? normalized : getLatestPostTeamChallengeTargets();
  }

  function isPostTeamChallengeTargetWithinSnapshot(target, snapshot){
    const t = normalizePostTeamChallengeTarget(target);
    ...
    return t.row >= 0 && t.col >= 0 && t.row < rows && t.col < cols;
  }

  function buildPriorityDetailFetchJobs(detailJobs, priorityTargets, snapshot){
    const out = [];
    const seen = new Set();
    const push = (row, col) => {
      const r = Number(row);
      ...
      if(seen.has(key)) return;
      seen.add(key);
      out.push({ row: Math.floor(r), col: Math.floor(c) });
    };

    const targets = normalizePostTeamChallengeTargets(priorityTargets);
    // 保持は「古い→新しい」順。詳細取得は「新しい→古い」順で先頭へ差し込む。
    for(const target of targets.slice().reverse()){
      if(target && isPostTeamChallengeTargetWithinSnapshot(target, snapshot)){
        // 最新クリック対象群は、差分の有無にかかわらず先頭で詳細取得する。
        // 連打時の体感では「最後に触った近辺」の反映が重要。
        push(target.row, target.col);
      }
    }

    for(const job of (Array.isArray(detailJobs) ? detailJobs : [])){
      if(!job) continue;
      push(job.row, job.col);
    }
  ...
  function requestPostTeamChallengeSync(kind, targets){
    const syncKind = sanitizePostTeamChallengeSyncKind(kind);
    const syncTargets = normalizePostTeamChallengeTargets(targets);
    ...
    if(syncTargets.length > 0){
      DBA_POST_TEAMCHALLENGE_SYNC.latestTargets = syncTargets;
    }
    DBA_POST_TEAMCHALLENGE_SYNC.seq += 1;

    if(DBA_POST_TEAMCHALLENGE_SYNC.timer){
      clearTimeout(DBA_POST_TEAMCHALLENGE_SYNC.timer);
      DBA_POST_TEAMCHALLENGE_SYNC.timer = 0;
    }
    ...
    return new Promise((resolve) => {
      const delayMs = getPostTeamChallengeSyncDelayMs();
      DBA_POST_TEAMCHALLENGE_SYNC.timer = setTimeout(() => {
        DBA_POST_TEAMCHALLENGE_SYNC.timer = 0;
        runLatestPostTeamChallengeSync()
          .then(resolve)
          .catch(() => resolve(false));
      }, delayMs);
    });
  ...
  async function runLatestPostTeamChallengeSync(){
    if(DBA_POST_TEAMCHALLENGE_SYNC.running){
      DBA_POST_TEAMCHALLENGE_SYNC.pending = true;
      return true;
    }
    ...
    try{
      // 連打中に完全な無限ループへ入らないよう、1回の起動では最大4回まで。
      // それ以上の追加要求は finally 側で、改めてデバウンスして最新1件だけ処理する。
      for(let guard = 0; guard < 4; guard += 1){
        DBA_POST_TEAMCHALLENGE_SYNC.pending = false;
        const startedSeq = DBA_POST_TEAMCHALLENGE_SYNC.seq;
        const kind = sanitizePostTeamChallengeSyncKind(DBA_POST_TEAMCHALLENGE_SYNC.lastKind);
        const targets = getLatestPostTeamChallengeTargets();

        if(kind === 'diff'){
          ok = await runBattleInfoSyncAfterTeamChallengeOnce(targets);
        }else{
          ok = await runBattlemapRefreshWithoutCellDetailsAfterTeamChallengeOnce(targets);
        }

        if(
          !DBA_POST_TEAMCHALLENGE_SYNC.pending &&
          DBA_POST_TEAMCHALLENGE_SYNC.seq === startedSeq
        ){
          return ok;
        }

        // 追加クリックが来ていても即再実行しない。
        // 少し待ってから、その時点の最新状態だけを再取得する。
        DBA_POST_TEAMCHALLENGE_SYNC.pending = false;
        await sleepMs(getPostTeamChallengeSyncDelayMs());
      }
      ...
      if(DBA_POST_TEAMCHALLENGE_SYNC.pending){
        DBA_POST_TEAMCHALLENGE_SYNC.pending = false;
        requestPostTeamChallengeSync(
          DBA_POST_TEAMCHALLENGE_SYNC.lastKind,
          getLatestPostTeamChallengeTargets()
        ).catch(() => {});
      }
    }
  }

  async function runBattleInfoSyncAfterTeamChallengeOnce(priorityTargets){
    const btn = document.getElementById('dba-btn-battleinfo');
    if(!btn) return false;
    ...
    try{
      if(mode === 'rb'){
        const live = await fetchRbLiveMapSnapshotOnce('teamchallenge:battle-info-click');
        if(live && live.snapshot){
          await updateOnlyChangedCellsFromFetchedDoc(null, {
  ...
  async function scheduleBattleInfoSyncAfterTeamChallenge(priorityTargets){
    return requestPostTeamChallengeSync('diff', priorityTargets);
  }
  ...
  function triggerLightRefreshNow(reason){
    ...
    DBA_LIGHT_REFRESH_STATE.reason = String(reason || 'manual');
    ...
    DBA_LIGHT_REFRESH_STATE.timer = setTimeout(() => {
      try{
        const url = makeTeambattleUrl({
          ...
          dba_light_refresh_reason: DBA_LIGHT_REFRESH_STATE.reason || 'manual'
        });
        location.replace(url);
      }catch(_e){
        location.reload();
      }
    }, DBA_LIGHT_REFRESH_STATE.reason === 'auto-lag' ? 50 : 600);

    return true;
  }

  async function scheduleAutoBattleInfoSyncAfterBattleResult(priorityTargets){
    const s = loadSettings();
  ...
  async function runBattlemapRefreshWithoutCellDetailsAfterTeamChallengeOnce(priorityTargets){
  ...
  async function scheduleBattlemapRefreshWithoutCellDetailsAfterTeamChallenge(priorityTargets){
    return requestPostTeamChallengeSync('map', priorityTargets);
  }
  ...
  async function scheduleBattleInfoDiffSyncAfterBattleLog(){
    ...
    try{
      do{
        ...
        if(mode === 'rb'){
          const live = await fetchRbLiveMapSnapshotOnce('battlelog:battle-info-click');
          if(live && live.snapshot){
            await updateOnlyChangedCellsFromFetchedDoc(null, {
              topUrl: '/map/rb/static',
              snapshot: live.snapshot,
  ...
  async function rapidAttackAt(row, col){
    ...
    const priorityTargets = rememberLatestPostTeamChallengeTarget(
      row,
      col,
      'rapid-attack'
    );
    ...
    scheduleDeferredLocalOwnershipCountsUpdate('teamchallenge:rapid-attack');
  ...
  function loadSettings(){
    try{
      const raw = dbaStorageGetItem(LS_KEY);
      if(!raw){
  ...
  function saveSettings(s){
    try{
      ...
      dbaStorageSetItem(LS_KEY, JSON.stringify(out));
  ...
  function getModeLabel(m){
    if(m === 'rb') return 'レッドvsブルー';
    if(m === 'hc') return 'ハードコア';
    if(m === 'l') return 'ラダー';
    return m;
  }
  ...
  function getRbGridSizeFromPageScript(docLike){
    // RBページの IIFE スクリプト内に "const GRID_SIZE = N;" がある想定。
    // v9.0.0.0: 公式側のマップサイズ可変化が進んだため、
    //             script → DBAスナップショット → canvas実寸 の順で安全にフォールバックする。
    const doc = (docLike && typeof docLike.querySelectorAll === 'function') ? docLike : document;

    const parseSizeFromText = (text) => {
      const t = String(text || '');
      if(!t || !t.includes('GRID_SIZE')) return null;
      const m = t.match(/(?:const|let|var)\s+GRID_SIZE\s*=\s*(\d+)\s*;/);
      if(!m) return null;
      const n = Number(m[1]);
    ...
    try{
      ...
      // まずはRB公式描画IIFEらしい script を優先する。
      for(const sc of scripts){
        const t = sc.textContent || '';
        if(!t) continue;
        if(
          t.includes('GRID_SIZE') &&
          (t.includes('modeQS') || t.includes('gridWrap') || t.includes('gridOverlay') || t.includes('cellColors'))
        ){
          const n = parseSizeFromText(t);
          if(n) return n;
        }
      }

      // 念のため、条件を緩めて全scriptから拾う。
      for(const sc of scripts){
        const n = parseSizeFromText(sc && sc.textContent);
        if(n) return n;
      }
    }catch(_e){}
    ...
    try{
      if(doc === document){
        const snap = DBA_BATTLEMAP_SNAPSHOT_CACHE && DBA_BATTLEMAP_SNAPSHOT_CACHE.snapshot;
        const n = Number(snap && snap.rows);
        if(Number.isFinite(n) && n > 0 && Number(snap && snap.cols) === n) return n;
      }
    }catch(_e){}

    try{
      const overlay = doc.getElementById ? doc.getElementById('gridOverlay') : null;
      if(overlay && Number(overlay.width) > 0 && Number(overlay.height) > 0){
        // 公式canvasは正方形マップなので、script抽出に失敗した時だけ実寸から推定する。
        const cssW = Number(overlay.clientWidth || 0);
        const rawW = Number(overlay.width || 0);
        const dpr = cssW > 0 ? (rawW / cssW) : (window.devicePixelRatio || 1);
        const approxCell = Math.max(16, Math.min(64, Math.round(32 * (dpr || 1))));
        const n = Math.round(rawW / approxCell);
  ...
  function ensureRbPointerFix(){
    ...
    function calcRC(evt){
      ...
      const cellW = rect.width / GRID_SIZE;  // 見た目上のセル幅
      const cellH = rect.height / GRID_SIZE; // 見た目上のセル高
      if(!Number.isFinite(cellW) || !Number.isFinite(cellH) || cellW <= 0 || cellH <= 0) return null;
      const c = Math.floor(rx / cellW);
      const r = Math.floor(ry / cellH);
    ...
    function resetLPState(){
      clearLP();
      lpFired = false;
      lpCtx = null;
    }

    function makeLpCtxSnapshot(ctx){
      if(!ctx) return null;
      return {
        r: Number(ctx.r),
        c: Number(ctx.c),
    ...
    function onPointerDown(evt){
      ...
      try{
        if(evt.type === 'pointerdown' && typeof overlay.setPointerCapture === 'function' && Number.isFinite(evt.pointerId)){
      ...
      const ctx = makeLpCtxSnapshot(lpCtx);
      const seq = ++lpSeq;
      longPressGaugeStart(lpCtx.x, lpCtx.y, LP_MS);
      lpTimer = setTimeout(async () => {
        if(!isLpSeqCurrent(seq)) return;
        lpTimer = 0;
        lpFired = true;
        try{
          longPressGaugeComplete();
          if(ctx){
            await handleAutoEquipLongPress(ctx.r, ctx.c, ctx.x, ctx.y);
          }
        }catch(_e){
          // noop
        }
    ...
    function onPointerUp(evt){
      ...
      const ctx = makeLpCtxSnapshot(lpCtx) || calcRC(evt);
      const fired = lpFired;
      resetLPState();
      if(!ctx) return;
      if(fired) return;
    ...
    overlay.addEventListener('pointercancel', (e)=>{ resetLPState(); }, true);
    ...
    overlay.addEventListener('touchcancel', (e)=>{ resetLPState(); }, { capture:true, passive:false });
    window.addEventListener('pointerup', (evt) => {
      // overlay 外でボタンを離した場合でも、古い長押し判定を残さない。
      resetLPState();
    }, false);
    ...
    document.addEventListener('visibilitychange', () => {
      if(document.hidden) resetLPState();
    });
  ...
  function buildBattleResultOptionsModal(){
    ...
    autoCloseWrap.className = 'dba-ae-opt-row';
    autoCloseWrap.id = 'dba-br-opt-autoclose-wrap';
    autoCloseWrap.style.marginTop = '12px';

    const autoCloseChk = document.createElement('input');
    autoCloseChk.type = 'checkbox';
    autoCloseChk.id = 'dba-br-opt-autoclose';

    const autoCloseText1 = document.createElement('span');
    autoCloseText1.textContent = '戦闘ログを';

    const autoCloseSecInp = document.createElement('input');
    autoCloseSecInp.type = 'number';
    autoCloseSecInp.min = '1.0';
    autoCloseSecInp.step = '0.1';
    autoCloseSecInp.id = 'dba-br-opt-autoclose-sec';
    autoCloseSecInp.style.width = '78px';

    const autoCloseText2 = document.createElement('span');
    autoCloseText2.textContent = '秒で自動的に閉じる。';

    autoCloseWrap.appendChild(autoCloseChk);
    autoCloseWrap.appendChild(autoCloseText1);
    autoCloseWrap.appendChild(autoCloseSecInp);
    autoCloseWrap.appendChild(autoCloseText2);
    mid.appendChild(autoCloseWrap);
    ...
    autoCloseChk.checked = loadBattleResultAutoCloseEnabled();
    autoCloseSecInp.value = loadBattleResultAutoCloseSec().toFixed(1);
    ...
    autoCloseChk.addEventListener('change', (e) => {
      e.stopPropagation();
      saveBattleResultAutoCloseEnabled(!!autoCloseChk.checked);
      saveBattleResultAutoCloseSec(autoCloseSecInp.value);
      autoCloseSecInp.value = loadBattleResultAutoCloseSec().toFixed(1);
      rearmBattleResultAutoCloseIfOpen();
    });

    const handleAutoCloseSecChange = (e) => {
      e.stopPropagation();
      saveBattleResultAutoCloseSec(autoCloseSecInp.value);
      autoCloseSecInp.value = loadBattleResultAutoCloseSec().toFixed(1);
      rearmBattleResultAutoCloseIfOpen();
    };
    autoCloseSecInp.addEventListener('change', handleAutoCloseSecChange);
    autoCloseSecInp.addEventListener('blur', handleAutoCloseSecChange);
  ...
  function openBattleResultOptionsModal(){
    ...
    const autoCloseChk = dlg.querySelector('#dba-br-opt-autoclose');
    if(autoCloseChk) autoCloseChk.checked = loadBattleResultAutoCloseEnabled();
    const autoCloseSecInp = dlg.querySelector('#dba-br-opt-autoclose-sec');
    if(autoCloseSecInp) autoCloseSecInp.value = loadBattleResultAutoCloseSec().toFixed(1);
  ...
  function openBattleResultModalWithNode(nodeOrText, titleText){
    ...
    // クリック透過ON：
    // 「戦闘結果：オプション」の保存値だけに従う。
    // RBのラピッド攻撃／公式RB action hookなど、NonBlocking経由の表示でも
    // チェックOFFなら通常dialog表示、チェックONなら透過フロート表示にする。
    const pass = loadBattleResultPassThrough();
    if(pass){
    ...
    const autoCloseText =
      (typeof nodeOrText === 'string')
        ? nodeOrText
        : ((nodeOrText instanceof Node) ? sanitizeText(nodeOrText.textContent || '') : '');
    if(shouldStartBattleResultAutoClose(autoCloseText)){
      scheduleBattleResultAutoClose();
    }else{
      cancelBattleResultAutoCloseTimer();
    }
  ...
  async function fetchFormAndShowBattleResult(form, submitter){
    ...
    let priorityTargets = [];
    {
      ...
      if(isTeamChallengeAction && consumeOwnCapitalAttackIfNeeded(row, col)){
        return;
      }
      if(isTeamChallengeAction){
        priorityTargets = rememberLatestPostTeamChallengeTarget(
          row,
          col,
          'form-submit'
        ) || [];
    ...
    if(isTeamChallengeAction){
      scheduleDeferredLocalOwnershipCountsUpdate('teamchallenge:form-result');
    }
  ...
  //  - a[href*="teambattle?r="] があれば URLから r,c を読む
  //  - それが無い場合は .cell の index と列数 から r,c を推定
  function initBattlemapCellClickIntercept(){
    ...
    function onUp(evt){
      ...
      const rc = makeLpCtxSnapshot(lpCtx) || getRCFromEvent(evt);
      const fired = lpFired;
      // ★重要：
  ...
  let dbaLayerRAF = 0;
  let dbaLayerScrollSyncTimer = 0;
  let dbaLayerResizeSyncTimer = 0;
  ...
  const DBA_LAYER_RESIZE_SETTLE_DELAY_MS = 750;
  // light reload 時にセル内テキストの再表示待機フレーム数
  const DBA_LAYER_TEXT_SHOW_DELAY_FRAMES = 2;
  let dbaLayerShowRAFs = [];

  function clearDeferredBattlemapLayerScrollSync(){
    if(dbaLayerScrollSyncTimer){
      clearTimeout(dbaLayerScrollSyncTimer);
      dbaLayerScrollSyncTimer = 0;
    }
  }

  function clearDeferredBattlemapLayerResizeSync(){
    if(dbaLayerResizeSyncTimer){
      clearTimeout(dbaLayerResizeSyncTimer);
      dbaLayerResizeSyncTimer = 0;
    }
  }

  function scheduleBattlemapLayerSyncAfterScrollSettled(){
    // スクロール中は position:fixed レイヤーの再配置を追従させない。
    // 最後のスクロールイベントから少し待って、表示位置が落ち着いてから1回だけ同期する。
    // これにより、fetch監視・現在地キャッシュ更新などの内部処理は止めず、
    // 画面描画だけをスクロール中の高頻度実行から切り離す。
    clearDeferredBattlemapLayerScrollSync();
    dbaLayerScrollSyncTimer = setTimeout(() => {
      dbaLayerScrollSyncTimer = 0;
      resetBattlemapLayerRectStabilizer();
      scheduleBattlemapLayerSync();
    }, DBA_LAYER_SCROLL_SETTLE_DELAY_MS);
  }

  function scheduleBattlemapLayerSyncAfterResizeSettled(){
    // バトルマップの縦横サイズが変化している間は、透明レイヤーのサイズを
    // リアルタイム追従させない。最後のサイズ変化から少し待って1回だけ同期する。
    clearDeferredBattlemapLayerResizeSync();
    dbaLayerResizeSyncTimer = setTimeout(() => {
      dbaLayerResizeSyncTimer = 0;
      resetBattlemapLayerRectStabilizer();
      scheduleBattlemapLayerSync();
    }, DBA_LAYER_RESIZE_SETTLE_DELAY_MS);
  }
      ...
      try{
        ...
        await mapLimit(jobs, CONCURRENCY, async (job) => {
          ...
          try{
            ...
            rememberFetchedCellDetailText(getCurrentBattlemapSnapshot(), row, col, holder, reg);
          }catch(_e){
  ...
  function initBattlemapLayer(){
    ...
    const start = () => {
      ...
      // スクロール・リサイズ追従
      // - スクロール：描画追従は重くなりやすいため、スクロールが落ち着いてから再同期
      // - リサイズ：バトルマップの縦横サイズが変化している間は追従せず、落ち着いてから再同期
      window.addEventListener('scroll', scheduleBattlemapLayerSyncAfterScrollSettled, { passive: true });
      window.addEventListener('resize', scheduleBattlemapLayerSyncAfterResizeSettled, { passive: true });
      window.addEventListener('orientationchange', scheduleBattlemapLayerSyncAfterResizeSettled, { passive: true });
      if(window.visualViewport){
        window.visualViewport.addEventListener('resize', scheduleBattlemapLayerSyncAfterResizeSettled, { passive: true });
        window.visualViewport.addEventListener('scroll', scheduleBattlemapLayerSyncAfterScrollSettled, { passive: true });
      }
      ...
      if(target && typeof ResizeObserver === 'function'){
        dbaLayerResizeObs = new ResizeObserver(() => scheduleBattlemapLayerSyncAfterResizeSettled());
  ...
  const DBA_RB_BUILD_NOTICE = {
    closeTimer: 0,
    outsideBound: false,
    alertHookInstalled: false,
    rawAlert: null,
    rawPageAlert: null,
    lastPointer: {
      x: 0,
  ...
  function normalizeRbBuildNoticeMessage(message){
    const raw = sanitizeText(message || '');
    if(!raw) return '';

    if(raw.includes('クールダウン中です') || /cool\s*down|cooldown/i.test(raw)){
      return 'クールダウン中です';
    }
    if(raw.includes('このタイルにはすでに建物があります') || /already.*build|building.*exist|exists/i.test(raw)){
      return 'このタイルにはすでに建物があります';
    }
    if(raw.includes('上限に達しました') || /limit|max/i.test(raw)){
      return '上限に達しました';
    }
    if(raw.includes('このタイルは所有していません') || /not.*own|not.*owned|ownership/i.test(raw)){
      return 'このタイルは所有していません';
    }

    return '';
  }

  function rememberRbBuildNoticePointerEvent(e){
    if(mode !== 'rb') return;
    ...
    DBA_RB_BUILD_NOTICE.lastPointer.x = x;
    DBA_RB_BUILD_NOTICE.lastPointer.y = y;
    DBA_RB_BUILD_NOTICE.lastPointer.at = Date.now();
  }

  function resolveRbBuildNoticePoint(clientX, clientY){
    ...
    const lastAt = Number(DBA_RB_BUILD_NOTICE.lastPointer.at || 0);
    if(lastAt > 0 && (Date.now() - lastAt) <= 5000){
      const lx = Number(DBA_RB_BUILD_NOTICE.lastPointer.x);
      const ly = Number(DBA_RB_BUILD_NOTICE.lastPointer.y);
  ...
  function ensureRbBuildNoticeDialog(){
    let dlg = document.getElementById('dba-m-rb-build-notice');
    if(dlg) return dlg;

    dlg = document.createElement('div');
    dlg.id = 'dba-m-rb-build-notice';
    dlg.dataset.open = '0';
    dlg.setAttribute('role', 'dialog');
    dlg.setAttribute('aria-modal', 'false');

    const mid = document.createElement('div');
    mid.className = 'dba-rb-build-notice__mid';
    mid.id = 'dba-m-rb-build-notice-message';

    const bot = document.createElement('div');
    bot.className = 'dba-rb-build-notice__bot';

    const btnOk = document.createElement('button');
    btnOk.type = 'button';
    btnOk.className = 'dba-rb-build-notice__ok';
    btnOk.textContent = 'OK';
    btnOk.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      closeRbBuildNoticeDialog();
    });
    ...
    if(!DBA_RB_BUILD_NOTICE.outsideBound){
      DBA_RB_BUILD_NOTICE.outsideBound = true;
      document.addEventListener('pointerdown', (e) => {
        const current = document.getElementById('dba-m-rb-build-notice');
        if(!current || current.dataset.open !== '1') return;
        const t = e.target;
        if(t instanceof Node && current.contains(t)) return;

        if(DBA_RB_BUILD_NOTICE.closeTimer){
          clearTimeout(DBA_RB_BUILD_NOTICE.closeTimer);
          DBA_RB_BUILD_NOTICE.closeTimer = 0;
        }
        DBA_RB_BUILD_NOTICE.closeTimer = setTimeout(() => {
          DBA_RB_BUILD_NOTICE.closeTimer = 0;
          closeRbBuildNoticeDialog();
        }, 350);
      }, true);
  ...
  function closeRbBuildNoticeDialog(){
    if(DBA_RB_BUILD_NOTICE.closeTimer){
      clearTimeout(DBA_RB_BUILD_NOTICE.closeTimer);
      DBA_RB_BUILD_NOTICE.closeTimer = 0;
    }
    const dlg = document.getElementById('dba-m-rb-build-notice');
    if(!dlg) return;
    dlg.dataset.open = '0';
  }

  function placeRbBuildNoticeNearPoint(dlg, x, y){
    if(!(dlg instanceof HTMLElement)) return;

    const vw = window.innerWidth || document.documentElement.clientWidth || 800;
    const vh = window.innerHeight || document.documentElement.clientHeight || 600;
    const point = resolveRbBuildNoticePoint(x, y);
    const px = Number.isFinite(Number(point.x)) ? Number(point.x) : (vw / 2);
    const py = Number.isFinite(Number(point.y)) ? Number(point.y) : (vh / 2);
    const margin = 10;
    const gap = 14;
    ...
    const h = Math.max(1, rect.height || 120);

    let left = (px < vw / 2) ? (px + gap) : (px - w - gap);
    let top = (py < vh / 2) ? (py + gap) : (py - h - gap);
  ...
  function openRbBuildNoticeDialog(message, clientX, clientY){
    const normalized = normalizeRbBuildNoticeMessage(message);
    if(!normalized) return false;

    const dlg = ensureRbBuildNoticeDialog();
    const mid = document.getElementById('dba-m-rb-build-notice-message');
    if(mid) mid.textContent = normalized;
    ...
    placeRbBuildNoticeNearPoint(dlg, clientX, clientY);

    const btnOk = dlg.querySelector('.dba-rb-build-notice__ok');
    if(btnOk instanceof HTMLElement){
      try{ btnOk.focus({ preventScroll:true }); }catch(_e){}
    }

    return true;
  }

  function handleRbBuildNoticeAlertMessage(message, clientX, clientY){
    const normalized = normalizeRbBuildNoticeMessage(message);
    if(!normalized) return false;
    openRbBuildNoticeDialog(normalized, clientX, clientY);
    return true;
  }

  function installRbBuildNoticeAlertHook(){
    if(mode !== 'rb') return false;
    if(DBA_RB_BUILD_NOTICE.alertHookInstalled) return true;
    DBA_RB_BUILD_NOTICE.alertHookInstalled = true;

    try{
      document.addEventListener('pointerdown', rememberRbBuildNoticePointerEvent, true);
      document.addEventListener('click', rememberRbBuildNoticePointerEvent, true);
    }catch(_e){}

    try{
      const raw = (typeof window.alert === 'function')
        ? window.alert.bind(window)
        : null;

      DBA_RB_BUILD_NOTICE.rawAlert = raw;

      if(raw){
        window.alert = function(message){
          try{
            if(handleRbBuildNoticeAlertMessage(message)){
              return;
            }
          }catch(_e){}
          return raw(message);
    ...
    try{
      const pageWin = (typeof unsafeWindow !== 'undefined' && unsafeWindow)
        ? unsafeWindow
        : null;

      if(pageWin && pageWin !== window && typeof pageWin.alert === 'function'){
        const rawPage = pageWin.alert.bind(pageWin);
        DBA_RB_BUILD_NOTICE.rawPageAlert = rawPage;

        pageWin.alert = function(message){
          try{
            if(handleRbBuildNoticeAlertMessage(message)){
              return;
            }
          }catch(_e){}
          return rawPage(message);
        };
      }
    }catch(_e){}
  ...
  installRbBuildNoticeAlertHook();

  async function buildOnRbTileFromPopup(row, col, kind, clientX, clientY){
    if(mode !== 'rb') return false;
    ...
    try{
      ...
      if(!(res.ok && data && data.ok)){
        ...
        if(openRbBuildNoticeDialog(msg, clientX, clientY)){
          return false;
        }
        alert(msg);
        return false;
  ...
  function placePopupNearPoint(pop, x, y){
    ...
    const margin = 8;
    const gap = 12;
    const px = Number.isFinite(Number(x)) ? Number(x) : Math.floor(vw / 2);
    const py = Number.isFinite(Number(y)) ? Number(y) : Math.floor(vh / 2);
    ...
    // 表示領域の下限付近で下にはみ出す場合は、
    // ポップアップの下辺がビューポート下辺より少し上に収まるように引き上げる。
  ...
  async function autoEquipThenAttack(row, col, presetName){
    ...
    try{
      ...
      if(!result || result.ok !== true){
        throw new Error('auto equip verify failed');
      }
  ...
  async function handleAutoEquipLongPress(row, col, clientX, clientY){
    ...
    if(canTileOps){
      ...
      const tileOpRadioName = `dba-ae-tile-op-${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const tileOpItems = [];
      let tileOpBusy = false;

      const refreshTileOpButtons = () => {
        for(const item of tileOpItems){
          const selected = !!item.radio.checked;
          item.button.disabled = tileOpBusy || !selected;
          item.button.dataset.selected = selected ? '1' : '0';
        }
      };

      const makeTileOpRow = (kind, label) => {
        const opRow = document.createElement('div');
        opRow.className = 'dba-ae-tile-op-row';

        const radio = document.createElement('input');
        radio.type = 'radio';
        radio.name = tileOpRadioName;
        radio.className = 'dba-ae-tile-op-radio';
        radio.setAttribute('aria-label', `${label}を選択`);

        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'dba-ae-pop-btn dba-ae-tile-op-btn';
        button.textContent = label;
        button.disabled = true;
        button.dataset.selected = '0';

        radio.addEventListener('change', () => {
          refreshTileOpButtons();
        });

        button.addEventListener('click', async (e) => {
          e.preventDefault();
          e.stopPropagation();
          if(tileOpBusy || !radio.checked || button.disabled) return;

          tileOpBusy = true;
          refreshTileOpButtons();
          try{
            await buildOnRbTileFromPopup(row, col, kind, e.clientX, e.clientY);
          }finally{
            tileOpBusy = false;
            refreshTileOpButtons();
          }
        ...
        tileOpItems.push({ radio, button });
        opRow.appendChild(radio);
        opRow.appendChild(button);
        return opRow;
      };

      pop.appendChild(makeTileOpRow('f', '要塞を建設'));
      pop.appendChild(makeTileOpRow('r', 'レーダーを設置'));
      refreshTileOpButtons();
    }
  ...
  function migrateSaveDataToGen3IfNeeded(){
    try{
      ...
      try{ dbaStorageRemoveItem(LS_AUTO_EQUIP_CAND_KEY); }catch(_e){}
      try{ dbaStorageRemoveItem(LS_AUTO_EQUIP_LAST_KEY); }catch(_e){}
  ...
  function loadRosterStore(){
    try{
      const raw = dbaStorageGetItem(ROSTER_LS_KEY);
      if(!raw){
  ...
  function normalizeEquipVerifyText(v){
  ...
  function collectEquipVerifyListTexts(root){
    if(!(root instanceof Element)) return [];
    const items = Array.from(root.querySelectorAll('li'))
      .map((li) => normalizeEquipVerifyText(li.textContent))
      .filter(Boolean);
    if(items.length > 0) return items;
    const single = normalizeEquipVerifyText(root.textContent);
    return single ? [single] : [];
  }

  function findFirstTableAfterHeadingText(doc, headingText){
    if(!(doc instanceof Document)) return null;
    const hs = Array.from(doc.querySelectorAll('h1,h2,h3,h4,h5,h6'));
    const target = hs.find((el) => normalizeEquipVerifyText(el.textContent).includes(String(headingText || '')));
    if(!target) return null;

    let node = target.nextElementSibling;
    while(node){
      if(node.tagName === 'TABLE') return node;
      const nested = node.querySelector ? node.querySelector('table') : null;
  ...
  function extractEquipVerifyNameAndTierFromCell(cell){
    if(!(cell instanceof Element)) return { name:'', tier:'' };
    const spans = cell.querySelectorAll('span');
    const name = normalizeEquipVerifyText(spans[0]?.textContent || cell.textContent || '');
    const tier = normalizeEquipVerifyText(spans[1]?.textContent || '');
    return { name, tier };
  }

  function extractEquippedWeaponIdFromDoc(doc){
    const table = findFirstTableAfterHeadingText(doc, '装備している武器');
    if(!(table instanceof Element)) return null;
    const a = table.querySelector('a[href*="/modify/weapon/view/"]');
    if(!(a instanceof HTMLAnchorElement)) return null;
    const m = String(a.getAttribute('href') || '').match(/\/modify\/weapon\/view\/(\d+)/);
    return m ? Number(m[1]) : null;
  }

  function extractEquippedArmorIdFromDoc(doc){
    const table = findFirstTableAfterHeadingText(doc, '装備している防具');
    if(!(table instanceof Element)) return null;
    const a = table.querySelector('a[href*="/modify/armor/view/"]');
    if(!(a instanceof HTMLAnchorElement)) return null;
    const m = String(a.getAttribute('href') || '').match(/\/modify\/armor\/view\/(\d+)/);
    return m ? Number(m[1]) : null;
  }

  function buildEquippedNecklaceSignatureFromDoc(doc){
    const table = findFirstTableAfterHeadingText(doc, '装備しているネックレス');
    if(!(table instanceof Element)) return null;
    ...
    const meta = extractEquipVerifyNameAndTierFromCell(cells[0]);
    const attrs = collectEquipVerifyListTexts(cells[1]).join('\n');
    const mod = normalizeEquipVerifyText(cells[2].textContent);
  ...
  function resolveEquippedNecklaceIdFromDoc(doc){
    const sig = buildEquippedNecklaceSignatureFromDoc(doc);
    if(!sig) return { id:null, resolved:false, ambiguous:false };

    const table = doc.querySelector('#necklaceTable');
    if(!(table instanceof Element)) return { id:null, resolved:false, ambiguous:false };
    ...
    for(const row of rows){
      ...
      const meta = extractEquipVerifyNameAndTierFromCell(cells[0]);
      const attrs = collectEquipVerifyListTexts(cells[3]).join('\n');
      const mod = normalizeEquipVerifyText(cells[4].textContent);
      ...
      const equipA = cells[1].querySelector('a[href*="/equip/"]');
      if(!(equipA instanceof HTMLAnchorElement)) continue;
      const m = String(equipA.getAttribute('href') || '').match(/\/equip\/(\d+)/);
      if(!m) continue;
  ...
  function extractEquippedTripleFromResponseText(text){
    const src = String(text || '');
    if(!src) return null;
    ...
    const necklace = resolveEquippedNecklaceIdFromDoc(doc);
    return {
      weaponId: extractEquippedWeaponIdFromDoc(doc),
      armorId: extractEquippedArmorIdFromDoc(doc),
      necklaceId: necklace.id,
      necklaceResolved: !!necklace.resolved,
      necklaceAmbiguous: !!necklace.ambiguous
    };
  }

  function isEquippedTripleMatchedState(state, triple){
    if(!state || !Array.isArray(triple)) return false;

    const weaponOk = (triple[0] == null) ? true : Number(state.weaponId) === Number(triple[0]);
    const armorOk  = (triple[1] == null) ? true : Number(state.armorId) === Number(triple[1]);
    const neckOk   = (triple[2] == null) ? true : Number(state.necklaceId) === Number(triple[2]);

    return weaponOk && armorOk && neckOk;
  }
  ...
  async function equipByIdAfterDelay(id, delayMs){
    if(delayMs > 0){
      await sleepMs(delayMs);
    }
    return await equipById(id);
  }

  async function equipByIdMonitored(id, delayMs){
    try{
      const r = await equipByIdAfterDelay(id, delayMs);
      return {
        ok: !!r?.ok,
        missing: !!r?.missing,
        text: String(r?.text || ''),
  ...
  async function equipPresetByName(name){
    ...
    // 送信順は 武器 → 防具 → 首 を維持しつつ、
    // 前の応答は待たず、少し間隔を空けて /equip を送る。
    const monitored = [
      { token: 0, promise: equipByIdMonitored(triple[0], 0) },
      { token: 1, promise: equipByIdMonitored(triple[1], DBA_AUTO_EQUIP_STAGGER_MS) },
      { token: 2, promise: equipByIdMonitored(triple[2], DBA_AUTO_EQUIP_STAGGER_MS * 2) }
    ];
    ...
    // 3応答とも「目標3部位一致」を確認できなかった場合は fail-safe で失敗扱い
    return {
      ok: false,
      missing: false,
      deleted: false,
  ...
  function isLegacyGenerationPickRow(tr, kind){
    // 武器／防具だけを対象にする。
    // DBE と同じ考え方で、名称セルの「アイテム名」部分の末尾に * があれば LEGACY とみなす。
    if(kind !== 'weapon' && kind !== 'armor') return false;

    const td0 = tr ? tr.querySelector('td') : null;
    if(!td0) return false;

    // DBE 側の dbeParseNameTd() と同じく、span[0] があればそこを名称本体として優先する。
    // span が無い場合は、セル本文から「【種別】」以降や改行以降を落として判定する。
    const firstSpan = td0.querySelector('span');
    const raw = sanitizeText(firstSpan ? firstSpan.textContent : (td0.innerText || td0.textContent || ''));
    const namePart = raw.split('\n')[0].split('【')[0].trim();
    if(!namePart) return false;

    return /\*+\s*$/u.test(String(namePart || '').normalize('NFKC'));
  }
  ...
  async function openPickItemModal(kind){
    ...
    let hideLegacy = loadPickHideLegacy(kind);
    const legacyFilterAvailable = (kind === 'weapon' || kind === 'armor');
    let modColIdx = -1;
    ...
    if(!srcTable){
      ...
      const applyPickFilters = () => {
        ...
        for(const tr of trs){
          ...
          const okLegacy = (!legacyFilterAvailable || !hideLegacy) ? true : (!isLegacyGenerationPickRow(tr, kind));
          const show = okRarity && okMod && okLegacy;
      ...
      // チェックボックス：LEGACY 非表示（武器／防具のみ）
      if(legacyFilterAvailable){
        const legacyWrap = document.createElement('label');
        legacyWrap.className = 'dba-pick-filterchk';
        legacyWrap.id = 'dba-pick-chk-legacy-wrap';

        const legacyChk = document.createElement('input');
        legacyChk.type = 'checkbox';
        legacyChk.id = 'dba-pick-chk-legacy';
        legacyChk.checked = !!hideLegacy;

        const legacyChkText = document.createElement('span');
        legacyChkText.textContent = 'LEGACY非表示';

        legacyWrap.appendChild(legacyChk);
        legacyWrap.appendChild(legacyChkText);
        filterBar.appendChild(legacyWrap);

        legacyChk.addEventListener('change', (e) => {
          e.stopPropagation();
          hideLegacy = !!legacyChk.checked;
          savePickHideLegacy(kind, hideLegacy);
          applyPickFilters();
        });
      }
  ...
  // =========================
  // 「保有地域数」ローカル更新：アクション後の遅延・単発実行
  //  - リアルタイム／高頻度更新は行わない
  //  - 戦況情報や /teamchallenge 系アクション後、描画が落ち着いた段階で1回だけ実行
  //  - 失敗した場合のみ追加で2回まで試行（合計最大3回）
  // =========================
  const DBA_LOCAL_OWNERSHIP_UPDATE = {
    timer: 0,
    seq: 0,
    running: false
  };

  function cancelDeferredLocalOwnershipCountsUpdate(){
    if(DBA_LOCAL_OWNERSHIP_UPDATE.timer){
      clearTimeout(DBA_LOCAL_OWNERSHIP_UPDATE.timer);
      DBA_LOCAL_OWNERSHIP_UPDATE.timer = 0;
    }
  }

  async function runLocalOwnershipCountsUpdateOnce(){
    const wrap = findTeamTablesWrap(document);
    if(!wrap) return false;
    ...
    const snap = getBattlemapSnapshotFromDoc(document);
    if(!snap || !Number.isFinite(Number(snap.rows)) || !Number.isFinite(Number(snap.cols))){
      return false;
    }

    await applyLocalOwnershipCountsToTeamTable(wrap, snap);
    return true;
  }

  function scheduleDeferredLocalOwnershipCountsUpdate(reason){
    void reason;

    // 同一アクション中に複数箇所から呼ばれても、最後の1回だけを採用する
    const seq = ++DBA_LOCAL_OWNERSHIP_UPDATE.seq;
    cancelDeferredLocalOwnershipCountsUpdate();

    DBA_LOCAL_OWNERSHIP_UPDATE.timer = setTimeout(async () => {
      DBA_LOCAL_OWNERSHIP_UPDATE.timer = 0;

      if(DBA_LOCAL_OWNERSHIP_UPDATE.running) return;
      DBA_LOCAL_OWNERSHIP_UPDATE.running = true;

      try{
        // マップ差し替え・script再実行・レイヤー同期の直後を避ける
        await raf2();

        for(let attempt = 1; attempt <= 3; attempt++){
          if(seq !== DBA_LOCAL_OWNERSHIP_UPDATE.seq) return;

          const ok = await runLocalOwnershipCountsUpdateOnce();
          if(ok) return;
        ...
        DBA_LOCAL_OWNERSHIP_UPDATE.running = false;
      }
    }, 900);
  }
  ...
  async function refreshHeaderAndTeamTablesFromFetchedDoc(doc, meta){
    ...
    try{
      ...
      // (B-2) 「保有地域数」のローカル併記はここでは実行しない。
      // - この関数は header / チーム情報テーブルの差し替えだけを担当する
      // - ローカル情報を利用した保有地域数更新は、
      //   戦況情報や /teamchallenge 系アクション後に
      //   scheduleDeferredLocalOwnershipCountsUpdate() で遅延・単発実行する
      ...
      if(mode === 'rb'){
        ...
        if(rbApi && typeof rbApi === 'object'){
          try{
            if(typeof rbApi.redrawOverlay === 'function'){
              rbApi.redrawOverlay();
            }
          }catch(_e){}
        }
  ...
  function scheduleHeaderAndTeamTablesRefreshFromTopPage(reason){
    if(mode !== 'rb') return false;
    window.setTimeout(() => {
      fetchLatestTopPageDoc().then(({ topUrl, doc }) => {
        const snap = getBattlemapSnapshotFromDoc(doc);
        return refreshHeaderAndTeamTablesFromFetchedDoc(doc, {
          topUrl,
          snapshot: snap
        });
      }).catch((e) => {
        dbgLog('battlemapRefresh', 'warn', 'deferred header/team-tables refresh failed', {
          reason: String(reason || ''),
          error: String(e && e.message ? e.message : e)
        });
      });
  ...
  async function fetchRbLiveMapSnapshotOnce(sourceLabel){
    if(mode !== 'rb') return null;
    ...
    try{
      if(controller){
        timeoutId = window.setTimeout(() => {
          try{ controller.abort(); }catch(_e){}
        }, Math.max(1000, Number(DBA_RB_LIVE_MAP_FAST_FETCH_TIMEOUT_MS || 0)));
      }

      const res = await fetch('/map/rb/static', {
        method: 'GET',
      ...
      const snap = buildRbSnapshotFromLiveMapPayload(payload);
      if(!snap) return null;

      return {
        payload,
        snapshot: snap,
        source: String(sourceLabel || 'rb-live-map-fast')
      };
    }catch(_e){
      return null;
    }finally{
  ...
  async function applyRbLiveMapSnapshotFast(newSnap, sourceLabel){
    if(mode !== 'rb') return null;
    ...
    const sizeChanged =
      !curSnap ||
      Number(curSnap.rows) !== Number(newSnap.rows) ||
      Number(curSnap.cols) !== Number(newSnap.cols);

    let changed = [];
    let canvasApplied = false;

    if(sizeChanged){
      canvasApplied = tryRefreshRbBattlemapViaCurrentApi(
        newSnap,
        `${String(sourceLabel || 'rb-live-map-fast')}:size-changed`
      );
    }else{
      changed = diffChangedCells(curSnap, newSnap);
      if(changed.length > 0){
        canvasApplied = applyRbBattlemapPartialUpdate(newSnap, changed);
      }else{
        setBattlemapSnapshotCache(newSnap, String(sourceLabel || 'rb-live-map-fast'));
        canvasApplied = true;
      }
    }
  ...
  function patchRbBattlemapScriptForDbaApi(scriptText){
    ...
    if(!src.includes('const GRID_SIZE') || !src.includes('function drawStatic()') || !src.includes('buildingsPayload')) return src;
    ...
    patched = patched.replace(
      ...
      `let borderPx = 2;
        let dbaAnimRaf = 0;
        let dbaDisposed = false;
        let dbaInstanceRoot = null;
        const dbaDisposables = [];
        function dbaTrackDisposer(fn){
          try{
            if(typeof fn === 'function') dbaDisposables.push(fn);
          }catch(_e){}
        }
        function dbaRunDisposers(){
          while(dbaDisposables.length > 0){
            const fn = dbaDisposables.pop();
            try{ fn(); }catch(_e){}
          }
        }
        function dbaTrackAddEvent(target, type, listener, options){
          try{
            if(!target || typeof target.addEventListener !== 'function') return false;
            target.addEventListener(type, listener, options);
            dbaTrackDisposer(function(){
              try{
        ...
        function dbaGetInstanceRoot(){
          try{
            if(dbaInstanceRoot) return dbaInstanceRoot;
            const w = document.getElementById('gridWrap');
            dbaInstanceRoot = w ? w.closest('.gridCanvasOuter') : null;
            return dbaInstanceRoot;
          }catch(_e){
            return null;
          }
        }
        function dbaIsInstanceDetached(){
          try{
            const root = dbaGetInstanceRoot();
            return !!(dbaDisposed || (root && !root.isConnected));
          }catch(_e){
            return !!dbaDisposed;
          }
        }
        function dbaShouldStopLoop(){
          return dbaIsInstanceDetached();
        }`
    );

    patched = patched.replace(
      'document.head.appendChild(style);',
      `style.setAttribute('data-dba-official-rb-style', '1');
    document.head.appendChild(style);
    dbaTrackDisposer(function(){
      try{
...

    {
      const eventPatches = [
        [
          `document.addEventListener('visibilitychange', ()=>{
      if(document.visibilityState === 'visible') refreshLiveMapNow(); else clearLivePollTimer();
    });`,
          `dbaTrackAddEvent(document, 'visibilitychange', ()=>{
      if(document.visibilityState === 'visible') refreshLiveMapNow(); else clearLivePollTimer();
    });`
        ],
        [
          `window.addEventListener('focus', refreshLiveMapNow);`,
          `dbaTrackAddEvent(window, 'focus', refreshLiveMapNow);`
        ],
        [
          `window.addEventListener('blur', clearLivePollTimer);`,
          `dbaTrackAddEvent(window, 'blur', clearLivePollTimer);`
        ],
        [
          `overlay.addEventListener('click', onPointer, {passive:true});`,
          `dbaTrackAddEvent(overlay, 'click', onPointer, {passive:true});`
        ],
        [
          `overlay.addEventListener('touchstart', onPointer, {passive:false});`,
          `dbaTrackAddEvent(overlay, 'touchstart', onPointer, {passive:false});`
        ],
        [
          `document.addEventListener('click', function(e){
      if(!menuVisible()) return;
      if(actionMenu.contains(e.target) || overlay.contains(e.target)) return;
      closeActionMenu();
    }, {passive:true});`,
          `dbaTrackAddEvent(document, 'click', function(e){
      if(!menuVisible()) return;
      if(actionMenu.contains(e.target) || overlay.contains(e.target)) return;
      closeActionMenu();
    }, {passive:true});`
        ...
        [
          ...
          `dbaTrackAddEvent(document, 'keydown', function(e){ if(e.key==='Escape') closeActionMenu(); });`
        ],
        [
          `btnAttack.addEventListener('click', function(){
      if(!activeTile) return;
      if(tileOwnedByMyTeam(activeTile.r, activeTile.c)){
        navTo(activeTile.r, activeTile.c);
      } else {
        attackTile();
      }
    });`,
          `dbaTrackAddEvent(btnAttack, 'click', function(){
      if(!activeTile) return;
      if(tileOwnedByMyTeam(activeTile.r, activeTile.c)){
        navTo(activeTile.r, activeTile.c);
      } else {
        ...
        [
          `btnInfo.addEventListener('click', function(){ if(activeTile) navTo(activeTile.r, activeTile.c); });`,
          `dbaTrackAddEvent(btnInfo, 'click', function(){ if(activeTile) navTo(activeTile.r, activeTile.c); });`
        ],
        [
          `btnFort.addEventListener('click', function(){ placeBuilding('f'); });`,
          `dbaTrackAddEvent(btnFort, 'click', function(){ placeBuilding('f'); });`
        ],
        [
          `btnRadar.addEventListener('click', function(){ placeBuilding('r'); });`,
          `dbaTrackAddEvent(btnRadar, 'click', function(){ placeBuilding('r'); });`
        ],
        [
          `btnCancel.addEventListener('click', function(){ closeActionMenu(); });`,
          `dbaTrackAddEvent(btnCancel, 'click', function(){ closeActionMenu(); });`
        ],
        [
          `window.addEventListener('resize', scheduleResize, {passive:true});`,
          `dbaTrackAddEvent(window, 'resize', scheduleResize, {passive:true});`
        ],
        [
          `window.addEventListener('resize', function(){ if(menuVisible()) closeActionMenu(); }, {passive:true});`,
          `dbaTrackAddEvent(window, 'resize', function(){ if(menuVisible()) closeActionMenu(); }, {passive:true});`
        ],
        [
          `window.addEventListener('orientationchange', scheduleResize, {passive:true});`,
          `dbaTrackAddEvent(window, 'orientationchange', scheduleResize, {passive:true});`
        ],
        [
          `window.addEventListener('orientationchange', function(){ if(menuVisible()) closeActionMenu(); }, {passive:true});`,
          `dbaTrackAddEvent(window, 'orientationchange', function(){ if(menuVisible()) closeActionMenu(); }, {passive:true});`
        ],
        [
          `window.visualViewport.addEventListener('resize', scheduleResize, {passive:true});`,
          `dbaTrackAddEvent(window.visualViewport, 'resize', scheduleResize, {passive:true});`
        ],
        [
          `window.visualViewport.addEventListener('scroll', scheduleResize, {passive:true});`,
          `dbaTrackAddEvent(window.visualViewport, 'scroll', scheduleResize, {passive:true});`
        ]
      ];

      for(const [from, to] of eventPatches){
    ...
    // 公式ページ新実装:
    //   /map/rb/static を LIVE_POLL_MS 間隔で取得して applyLiveMapPayload() に流す。
    // DBA側では、陣取り変化の反映を少し速めるため 4000ms → 1500ms に短縮する。
    // また、別ウィンドウ操作などで document.hasFocus() が false になっても、
    // タブが表示中なら live map 追従は続ける。
    patched = patched.replace(
      /const\s+LIVE_POLL_MS\s*=\s*4000\s*;/,
      `const LIVE_POLL_MS = 1500;`
    );

    patched = patched.replace(
      /function\s+livePollAllowed\s*\(\s*\)\s*\{\s*return\s+!livePollStopped\s*&&\s*document\.visibilityState\s*===\s*'visible'\s*&&\s*document\.hasFocus\s*\(\s*\)\s*;\s*\}/,
    ...
    // 公式ページ新実装:
    //   /map/rb/static → applyLiveMapPayload(payload)
    // によるcanvas自動更新後、DBA側の透明レイヤーや独自境界線も追従させる。
    // 公式canvas更新は先に完了させ、その後でDBA hookを呼ぶ。
    patched = patched.replace(
      /if\s*\(\s*terrainImg\s*&&\s*buildingImg\s*\)\s*\{\s*drawStatic\s*\(\s*\)\s*;\s*drawOverlay\s*\(\s*performance\.now\s*\(\s*\)\s*\)\s*;\s*\}\s*return\s+true\s*;/,
      `if(terrainImg && buildingImg){ drawStatic(); drawOverlay(performance.now()); }
    try{
      if(window.__DBA_RB_LIVE_MAP_HOOK && typeof window.__DBA_RB_LIVE_MAP_HOOK === 'function'){
        window.__DBA_RB_LIVE_MAP_HOOK(payload, 'official:applyLiveMapPayload');
      }
    }catch(_dbaHookErr){}
    return true;`
    );
    ...
    const injected = `
      ...
      function dbaDisposeInstance(){
        try{
          dbaDisposed = true;
          try{
            dbaRunDisposers();
          }catch(_dbaDisposersErr){}
          try{
            const styles = document.querySelectorAll('style[data-dba-official-rb-style="1"]');
            for(const st of Array.from(styles)){
              if(st && st.parentNode){
                st.parentNode.removeChild(st);
              }
            }
          }catch(_dbaStyleDisposeErr){}
          try{
            livePollStopped = true;
            if(typeof clearLivePollTimer === 'function'){
              clearLivePollTimer();
            }
          }catch(_livePollDisposeErr){}
          dbaStopLoopHard();
          return true;
        }catch(_e){
          return false;
  ...
  function prepareImportedBattlemapScripts(root){
    ...
    for(const sc of scripts){
      ...
      if(!txt.includes('buildingsPayload')) continue;
      const patched = patchRbBattlemapScriptForDbaApi(txt);
      sc.textContent = patched;
      try{
        const okLoop = patched.includes('if(dbaShouldStopLoop()) return;');
        const okMain = patched.includes('dbaAnimRaf = requestAnimationFrame(loop);');
  ...
  function buildRbAllCellsForApiSnapshot(snap){
  ...
  function buildRbFullRefreshCellMetaForApi(snap){
    const cells = buildRbAllCellsForApiSnapshot(snap);
    return cells.map((rc) => ({
      row: rc.row,
      col: rc.col,
      base: true,
  ...
  function tryRefreshRbBattlemapViaCurrentApi(newSnap, sourceLabel){
    try{
      if(mode !== 'rb') return false;
      ...
      const cells = buildRbAllCellsForApiSnapshot(newSnap);
      if(cells.length === 0) return false;

      const meta = buildRbFullRefreshCellMetaForApi(newSnap);
      ...
      if(ok){
        setBattlemapSnapshotCache(newSnap, String(sourceLabel || 'tryRefreshRbBattlemapViaCurrentApi'));
      }
      return ok;
    }catch(_e){
      return false;
  ...
  async function refreshBattlemapFromFetchedDoc(doc, meta){
    ...
    try{
      ...
      try{
        if(mode === 'rb' && window.__DBA_RB_API && typeof window.__DBA_RB_API.dispose === 'function'){
          window.__DBA_RB_API.dispose();
        }
      }catch(_e){}
  ...
  async function refreshBattlemapFromTopPage(){
    ...
    try{
      ...
      if(v.ok){
        ...
        scheduleDeferredLocalOwnershipCountsUpdate('battlemap:top-page');
    ...
    if(mode === 'rb'){
      ...
      const pushKnown = (k, fowVisible) => {
        ...
        out.push({
          row: Number(m[1]),
          col: Number(m[2]),
          fowVisible: !!fowVisible
        });
      };

      // RBのFOW上「現在見えているセル」を先に並べ、探索済みだが現在は見えていないセルは後回しにする。
      for(const k of Array.from(snapshot.visibleSet || [])) pushKnown(k, true);
      for(const k of Array.from(snapshot.exploredSet || [])) pushKnown(k, false);
      out.sort((a,b) => {
        const av = a.fowVisible ? 0 : 1;
        const bv = b.fowVisible ? 0 : 1;
        return (av - bv) || (a.row - b.row) || (a.col - b.col);
      });
      return out;
    }
  ...
  // =========================
  // RB 長押し戦況情報：セル詳細テキストキャッシュ
  //  - 変更のないセルは前回表示を再利用し、詳細ページ取得をスキップする
  //  - 取得が必要なセルは、画面内に見えているセル → FOW上可視セル → 探索済みセルの順に処理する
  // =========================
  const DBA_RB_CELL_DETAIL_TEXT_CACHE = {
    map: new Map(),
    generation: 0
  };

  function rbCellKeyComma(row, col){
    return `${Number(row)},${Number(col)}`;
  }

  function rbCellKeyDash(row, col){
    return `${Number(row)}-${Number(col)}`;
  }

  function pruneRbCellDetailTextCache(now){
    const currentNow = Number.isFinite(Number(now)) ? Number(now) : Date.now();
    const maxAge = Math.max(30 * 1000, Number(DBA_RB_CELL_DETAIL_CACHE_MAX_AGE_MS || 0));

    for(const [key, item] of DBA_RB_CELL_DETAIL_TEXT_CACHE.map.entries()){
      const updatedAt = Number(item && item.updatedAt || 0);
      if(!(updatedAt > 0) || (currentNow - updatedAt) > maxAge){
        DBA_RB_CELL_DETAIL_TEXT_CACHE.map.delete(key);
      }
    }

    const maxEntries = Math.max(100, Number(DBA_RB_CELL_DETAIL_CACHE_MAX_ENTRIES || 0));
    while(DBA_RB_CELL_DETAIL_TEXT_CACHE.map.size > maxEntries){
      const first = DBA_RB_CELL_DETAIL_TEXT_CACHE.map.keys().next();
      if(first.done) break;
      DBA_RB_CELL_DETAIL_TEXT_CACHE.map.delete(first.value);
    }
  }

  function clearRbCellDetailTextCache(reason){
    try{
      DBA_RB_CELL_DETAIL_TEXT_CACHE.map.clear();
      DBA_RB_CELL_DETAIL_TEXT_CACHE.generation += 1;
      dbgLog('battleInfo', 'info', 'RB cell detail text cache cleared', {
        reason: String(reason || ''),
        generation: DBA_RB_CELL_DETAIL_TEXT_CACHE.generation
      });
    }catch(_e){}
  }

  function buildRbCellDetailCacheSignature(snapshot, row, col){
    if(mode !== 'rb') return '';
    if(!snapshot || typeof snapshot !== 'object') return '';
    ...
    const key = rbCellKeyDash(r, c);
    const color = (snapshot.cellColors && Object.prototype.hasOwnProperty.call(snapshot.cellColors, key))
      ? String(snapshot.cellColors[key] || '')
      : '';
    const cap = (snapshot.capitalSet instanceof Set && snapshot.capitalSet.has(key)) ? '1' : '0';
    const visible = (snapshot.visibleSet instanceof Set && snapshot.visibleSet.has(key)) ? '1' : '0';
    const explored = (snapshot.exploredSet instanceof Set && snapshot.exploredSet.has(key)) ? '1' : '0';

    // terrainsKey / buildingsKey はグローバル署名として含める。
    // 建物や地形が変わった場合は安全側で該当試合の詳細キャッシュを広めに無効化する。
    return [
      String(snapshot.mode || mode),
      `${Number(snapshot.rows || 0)}x${Number(snapshot.cols || 0)}`,
      key,
      ...
      `vis:${visible}`,
      `exp:${explored}`,
      `terrain:${String(snapshot.terrainsKey || '')}`,
      `build:${String(snapshot.buildingsKey || '')}`
    ].join('|');
  }

  function rememberRbCellDetailText(snapshot, row, col, text){
    if(mode !== 'rb') return false;
    const sig = buildRbCellDetailCacheSignature(snapshot, row, col);
    if(!sig) return false;

    const t = String(text || '');
    if(!t || /^ERR\b/.test(t)) return false;

    pruneRbCellDetailTextCache(Date.now());
    DBA_RB_CELL_DETAIL_TEXT_CACHE.map.set(rbCellKeyComma(row, col), {
      sig,
      text: t,
      updatedAt: Date.now()
    });
  ...
  function getRbCachedCellDetailText(snapshot, row, col){
    if(mode !== 'rb') return null;
    const sig = buildRbCellDetailCacheSignature(snapshot, row, col);
    if(!sig) return null;

    const key = rbCellKeyComma(row, col);
    const item = DBA_RB_CELL_DETAIL_TEXT_CACHE.map.get(key) || null;
    if(!item || item.sig !== sig) return null;
  ...
  function seedRbCellDetailTextCacheFromLayerSnapshot(snapshot, textSnapshot){
    if(mode !== 'rb') return 0;
    if(!snapshot || !textSnapshot || typeof textSnapshot !== 'object') return 0;

    let n = 0;
    for(const [key, text] of Object.entries(textSnapshot)){
      const m = String(key || '').match(/^(\d+),(\d+)$/);
      if(!m) continue;
      const t = String(text || '');
      if(!t || /^ERR\b/.test(t)) continue;
      if(rememberRbCellDetailText(snapshot, Number(m[1]), Number(m[2]), t)) n += 1;
    }
    return n;
  }

  function rememberFetchedCellDetailText(snapshot, row, col, holder, reg){
  ...
  function getLayerCellScreenPriority(row, col){
    try{
      const content = dbaLayerCellContentMap.get(rbCellKeyComma(row, col)) || null;
      const cell = content && content.parentElement ? content.parentElement : null;
      if(!cell || typeof cell.getBoundingClientRect !== 'function'){
        return { screenVisible:false, distance: Number.MAX_SAFE_INTEGER };
      }
  ...
  function buildRbAllCellDetailFetchPlan(
    ...
    if(mode !== 'rb' || !newSnap || !Array.isArray(jobs)){
      out.fetchJobs = Array.isArray(jobs) ? jobs.slice() : [];
      return out;
    }
    ...
    if(sameSize){
      changedKeys = new Set(
        diffChangedCells(curSnap, newSnap).map((it) => rbCellKeyComma(it.row, it.col))
      );
    }
    ...
    for(const rawJob of jobs){
      ...
      const commaKey = rbCellKeyComma(row, col);
      ...
      out.fetchJobs.push({
        ...
        changed,
        screenVisible: !!screen.screenVisible,
        screenDistance: Number(screen.distance || Number.MAX_SAFE_INTEGER)
      });
    }

    out.fetchJobs.sort((a,b) => {
      const as = a.screenVisible ? 0 : 1;
      const bs = b.screenVisible ? 0 : 1;
      if(as !== bs) return as - bs;
      ...
      const ac = a.changed ? 0 : 1;
      const bc = b.changed ? 0 : 1;
      if(ac !== bc) return ac - bc;

      const ad = Number(a.screenDistance || Number.MAX_SAFE_INTEGER);
      const bd = Number(b.screenDistance || Number.MAX_SAFE_INTEGER);
      if(ad !== bd) return ad - bd;
  ...
  function extractRbFowStateFromDoc(doc){
    try{
      ...
      for(const s of scripts){
        ...
        if(!t) continue;
        if(t.includes('window.__FOW') && (t.includes('"explored"') || t.includes('"visible"') || t.includes('hasCapital'))){
          src = t;
          break;
        }
      }
      ...
      let obj = null;
      const objText = extractAssignedObjectLiteral(src, 'window.__FOW');
      if(objText){
        try{ obj = JSON.parse(objText); }catch(_e){ obj = null; }
      }
      ...
      if(!obj || typeof obj !== 'object'){
        ...
        obj = {
          explored: (mx && mx[1]) ? JSON.parse(mx[1]) : [],
          visible: (mv && mv[1]) ? JSON.parse(mv[1]) : [],
          hasCapital: !!(mh && mh[1] === 'true')
        };
      }

      return {
        visible: Array.isArray(obj.visible) ? obj.visible : [],
        explored: Array.isArray(obj.explored) ? obj.explored : [],
        hasCapital: !!obj.hasCapital
      };
    }catch(_e){
  ...
  function extractRbBuildingsPayloadFromDoc(doc){
    try{
      ...
      const objText = extractAssignedObjectLiteral(src, 'window.__BUILDINGS');
...
function extractRbAvatarsPayloadFromDoc(doc){
  try{
    ...
    const objText = extractAssignedObjectLiteral(src, 'window.__AVATARS');
    if(!objText) return null;
...
function avatarsKeyToMap(avatarsKey){
  ...
  async function scanAllCellsAndRenderFromFetchedDoc(doc, meta){
    ...
    try{
      ...
      if(mode === 'rb'){
        seedRbCellDetailTextCacheFromLayerSnapshot(curSnap, oldTexts);
      }
      ...
      if(
        mode === 'rb' &&
        meta?.preferLiveMapFastPath &&
        newSnap
      ){
        if(
          curSnap &&
          (Number(curSnap.rows) !== Number(newSnap.rows) || Number(curSnap.cols) !== Number(newSnap.cols))
        ){
          clearRbCellDetailTextCache('rb-longpress:size-changed');
        }

        const applied = await applyRbLiveMapSnapshotFast(
          newSnap,
          'battle-info-longpress:rb-live-map'
        );
        if(!applied){
          throw new Error('rb live map longpress refresh failed');
        }
        ...
        scheduleHeaderAndTeamTablesRefreshFromTopPage('battle-info-longpress:deferred-header');
        refreshed = true;
      }else{
        if(!doc) throw new Error('doc is null');
        if(
    ...
    if(mode === 'rb'){
      ...
      if(btn){
        btn.textContent = `戦況取得中…(${detailJobs.length}/${plan.total}件取得・${cachedCount}件再利用)`;
      }
    }

    const CONCURRENCY = (mode === 'rb') ? 12 : 16;
    ...
    if(mode === 'rb'){
      dbgLog('battleInfo', 'info', 'RB longpress detail fetch plan completed', {
        total: jobs.length,
        fetched: detailJobs.length,
        cached: cachedCount,
  ...
  function escapeRegExpLiteral(text){
    const specials = '\\^$.*+?()[]{}|';
    return String(text || '').replace(/./g, (ch) => specials.includes(ch) ? `\\${ch}` : ch);
  }

  function extractAssignedObjectLiteral(scriptText, assignName){
    const src = String(scriptText || '');
    const name = escapeRegExpLiteral(assignName || '');
    if(!src || !name) return '';
  ...
  function extractConstLiteral(scriptText, constName){
    ...
    //   const cellColors = { ... };
    //   let   cellColors = { ... };
    // 公式ページ側で live map 更新用に cellColors / capitalList / terrainsPayload が
    // const から let へ変更されたため、宣言種別を限定せずに抽出する。
    // 関数名は既存呼び出しとの互換のため extractConstLiteral のまま残す。
    const src = String(scriptText || '');
    if(!src) return null;

    const name = escapeRegExpLiteral(constName || '');
    ...
    const re1 = new RegExp(
      `${decl}\\s+${name}\\s*=\\s*([\\s\\S]*?);\\s*(?=(?:const|let|var|function)\\b|$)`
    );
    let m = src.match(re1);
    if(m) return (m[1] || '').trim();

    // 第2候補（後方互換）：
    // 旧来どおり「改行 or 文末」で終わる形も許容する。
    const re2 = new RegExp(
      `${decl}\\s+${name}\\s*=\\s*([\\s\\S]*?);\\s*(?:\\n|$)`
    );
    m = src.match(re2);
    if(m) return (m[1] || '').trim();
  ...
  function normalizeCellColors(obj){
    ...
    // 公式ページ新実装の colorForCellPayload() に合わせ、
    // { colors:[{r,c,color}, ...] } 形式も受けられるようにする。
    if(Array.isArray(obj.colors)){
      for(const it of obj.colors){
        if(!it || typeof it !== 'object') continue;
        const r = Number(it.r ?? it.row ?? it.x);
        const c = Number(it.c ?? it.col ?? it.y);
        if(!Number.isFinite(r) || !Number.isFinite(c)) continue;
        const key = `${r}-${c}`;
        const v = it.color ?? it.hex ?? null;
  ...
  // =========================
  // RB公式 live map payload 追従
  //  - 公式ページ新実装では /map/rb/static を取得して applyLiveMapPayload() で
  //    canvas を自動更新する。
  //  - DBA側の透明レイヤー、DBAオリジナル境界線、現在地マーカー、
  //    首都攻撃ブロッカー等も同じタイミングで追従させる。
  //  - 公式canvas更新そのものは妨げない。
  // =========================
  function buildRbSnapshotFromLiveMapPayload(payload){
    try{
      if(mode !== 'rb') return null;
      if(!payload || typeof payload !== 'object') return null;

      const size = Number(payload.gridSize ?? payload.grid_size ?? payload.size ?? payload.rows ?? payload.cols);
      if(!Number.isFinite(size) || size <= 0) return null;

      const fow = (payload.fow && typeof payload.fow === 'object')
        ? payload.fow
        : {
            visible: Array.isArray(payload.visible) ? payload.visible : [],
            explored: Array.isArray(payload.explored) ? payload.explored : [],
            hasCapital: !!payload.hasCapital
          };
      const capitalsObj =
        (payload.capitals && typeof payload.capitals === 'object') ? payload.capitals :
        (payload.capitalMap && typeof payload.capitalMap === 'object') ? payload.capitalMap :
        (Array.isArray(payload.capitalList) ? payload.capitalList : {});
      ...
      out.cellColors = normalizeCellColors(payload.gridColors ?? payload.cellColors ?? payload.colors ?? Object.create(null));
      out.capitalSet = normalizeCapitalSet(Array.isArray(capitalsObj) ? capitalsObj : Object.values(capitalsObj));
      out.terrainsKey = normalizeTerrainsPayload(payload.gridTerrains ?? payload.terrains ?? payload.terrain ?? null);
      out.buildingsKey = normalizeBuildingsPayload(payload.buildings ?? payload.buildingsPayload ?? null);
      out.avatarsKey = normalizeAvatarsPayload(payload.avatars ?? payload.avatarsPayload ?? null);
      ...
      out.visibleList = normalizeRcList(Array.isArray(fow.visible) ? fow.visible : []);
      out.exploredList = normalizeRcList(Array.isArray(fow.explored) ? fow.explored : []);
      out.visibleSet = new Set(out.visibleList.map((rc) => `${rc[0]}-${rc[1]}`));
      out.exploredSet = new Set(out.exploredList.map((rc) => `${rc[0]}-${rc[1]}`));
      out.hasCapital = !!fow.hasCapital;
  ...
  function handleRbLiveMapPayloadFromOfficial(payload, source){
    try{
      if(mode !== 'rb') return false;
      ...
      const cur = getCurrentBattlemapSnapshot();
      const sizeChanged =
        !cur ||
        Number(cur.rows) !== Number(snap.rows) ||
        Number(cur.cols) !== Number(snap.cols);

      let canvasApplied = false;
      let changed = [];

      if(cur && !sizeChanged){
        changed = diffChangedCells(cur, snap);
        if(changed.length > 0){
          canvasApplied = applyRbBattlemapPartialUpdate(snap, changed);
        }
      }else if(sizeChanged){
        canvasApplied = tryRefreshRbBattlemapViaCurrentApi(
          snap,
          'official-live-map:size-changed'
        );
      }

      setBattlemapSnapshotCache(snap, String(source || 'official-live-map'));

      if(sizeChanged){
        // 通常、公式 applyLiveMapPayload() は gridSize 不一致を拒否するが、
        // 将来仕様でサイズ変更が流れてきた場合にもDBAレイヤーだけは破綻させない。
        initBattlemapLayer();
        resetBattlemapLayerRectStabilizer();
      }
      ...
      if(!canvasApplied){
        try{
          const rbApi = window.__DBA_RB_API;
          if(rbApi && typeof rbApi.redrawOverlay === 'function'){
            rbApi.redrawOverlay();
          }
        }catch(_e){}
      }
      ...
      renderOwnCapitalRapidAttackBlockers(snap);
      scheduleBattlemapLayerSync();
      scheduleDeferredLocalOwnershipCountsUpdate('official-live-map');
  ...
  function installRbLiveMapPayloadBridge(){
    if(mode !== 'rb') return false;

    const hook = function(payload, source){
      return handleRbLiveMapPayloadFromOfficial(payload, source);
    };

    try{
      window.__DBA_RB_LIVE_MAP_HOOK = hook;
    }catch(_e){}

    try{
      if(typeof unsafeWindow !== 'undefined' && unsafeWindow){
        unsafeWindow.__DBA_RB_LIVE_MAP_HOOK = hook;
      }
    }catch(_e){}
  ...
  function deriveRbGridSizeFromSnapshotParts(parts){
    let max = -1;
    const bump = (row, col) => {
      const r = Number(row);
      const c = Number(col);
    ...
    try{
      for(const key of Object.keys(parts?.cellColors || {})){
        const m = String(key || '').match(/^(\d+)-(\d+)$/);
        if(m) bump(m[1], m[2]);
      }
    }catch(_e){}
    ...
    try{
      for(const key of Array.from(parts?.capitalSet || [])){
        const m = String(key || '').match(/^(\d+)-(\d+)$/);
        if(m) bump(m[1], m[2]);
      }
    }catch(_e){}
    ...
    try{
      const terr = terrainsKeyToMap(parts?.terrainsKey || '');
    ...
    try{
      const bld = buildingsKeyToMap(parts?.buildingsKey || '');
      for(const key of bld.keys || []){
        const m = String(key || '').match(/^(\d+)-(\d+)$/);
        if(m) bump(m[1], m[2]);
      }
    }catch(_e){}
    ...
    try{
      const av = avatarsKeyToMap(parts?.avatarsKey || '');
      for(const key of av.keys || []){
        const m = String(key || '').match(/^(\d+)-(\d+)$/);
        if(m) bump(m[1], m[2]);
      }
      if(av.selfAvatar) bump(av.selfAvatar.row, av.selfAvatar.col);
    }catch(_e){}

    try{
      const fow = parts?.fowState || {};
      for(const list of [fow.visible, fow.explored]){
  ...
  function getBattlemapSnapshotFromDoc(doc){
    ...
    if(mode === 'rb'){
      const t = pickScriptText(doc, ['GRID_SIZE', 'cellColors', 'terrainsPayload']);
      ...
      if(!Number.isFinite(size) || size <= 0){
        size = getRbGridSizeFromPageScript(doc) || deriveRbGridSizeFromSnapshotParts({
          cellColors: out.cellColors,
          capitalSet: out.capitalSet,
          terrainsKey: out.terrainsKey,
          buildingsKey: out.buildingsKey,
          avatarsKey: out.avatarsKey,
          fowState
        });
      }
  ...
  function countRbKnownFowCells(snap){
    const keys = new Set();
    const rows = Number(snap?.rows || 0);
    const cols = Number(snap?.cols || 0);

    const pushRc = (row, col) => {
      const r = Number(row);
      const c = Number(col);
      if(!Number.isFinite(r) || !Number.isFinite(c)) return;
      if(r < 0 || c < 0) return;
      if(rows > 0 && r >= rows) return;
      if(cols > 0 && c >= cols) return;
      keys.add(`${r}-${c}`);
    };
  ...
  function isRbFullyUnexploredSnapshot(snap){
    if(mode !== 'rb') return false;
    if(!snap) return false;
    ...
    // FOW上の既知セルが1つでもあれば「全面未踏」とは見なさない。
    if(countRbKnownFowCells(snap) > 0){
      return false;
    }

    // 所有色・首都・建造物が見えている場合も、全面未踏ではない。
    const colorCount = Object.keys(snap.cellColors || {}).length;
    if(colorCount > 0) return false;

    const hasCapital =
      !!snap.hasCapital ||
      !!(snap.capitalSet && typeof snap.capitalSet.size === 'number' && snap.capitalSet.size > 0);
    if(hasCapital) return false;

    const bld = buildingsKeyToMap(snap.buildingsKey || '');
    if((bld.keys || []).length > 0) return false;
  ...
  async function refreshBattlemapWithoutCellDetailsFromFetchedDoc(doc, meta){
    const btn = document.getElementById('dba-btn-battleinfo');
    const showAlertOnError = (meta && Object.prototype.hasOwnProperty.call(meta, 'showAlertOnError'))
      ? !!meta.showAlertOnError
      : true;
    ...
    try{
      if(mode === 'rb'){
        ...
        if(live && live.snapshot){
          ...
          const applied = await applyRbLiveMapSnapshotFast(
            live.snapshot,
            live.source || 'map-only:rb-live-map'
          );
          if(applied){
      ...
      dbgLog('battleInfo', 'warn', 'MAP ONLY update failed', { mode, error: String(e && e.message ? e.message : e) });
      if(btn){
        btn.disabled = false;
        btn.dataset.dbaBusy = '0';
        btn.innerHTML = '戦況<br>情報';
  ...
  async function updateOnlyChangedCellsFromFetchedDoc(doc, meta){
    ...
    try{
      // RBでは /map/rb/static の軽量JSONスナップショットを優先し、
      // 公式canvas/APIへ差分だけを流す。HTML全体の再取得・script再実行を避ける。
      if(mode === 'rb' && meta?.preferLiveMapFastPath && meta?.snapshot){
        const curSnap = getCurrentBattlemapSnapshot();
        const newSnap = meta.snapshot;

        if(curSnap.rows !== newSnap.rows || curSnap.cols !== newSnap.cols){
          if(btn) btn.textContent = '全更新（サイズ差）…';
          const applied = await applyRbLiveMapSnapshotFast(newSnap, 'battle-info-click:size-changed');
          if(!applied){
        ...
        const oldTexts = snapshotLayerTexts();
        const applied = await applyRbLiveMapSnapshotFast(newSnap, 'battle-info-click:rb-live-map');
        if(!applied){
        ...
        const detailJobs = buildPriorityDetailFetchJobs(
          buildDetailFetchJobsForChangedCells(curSnap, newSnap, changed),
          priorityTargets,
          newSnap
        );
        ...
        await mapLimit(detailJobs, CONCURRENCY, async (job, idx) => {
          ...
          try{
            const { holder, reg } = await fetchCellDetail(row, col);
            rememberFetchedCellDetailText(newSnap, row, col, holder, reg);
          }catch(_e){
            setLayerCellText(row, col, `ERR\n(${row},${col})`);
          }
          if(btn && (idx % 10 === 0)){
      ...
      // ただし、取得済みの最新マップが全面未踏なら、すでに次試合マップへ移っている可能性が高い。
      // この場合は「境界再確認中…」でボタンを長時間塞がず、そのままマップ差し替えへ進む。
      if(mode === 'rb' && changed.length === 0 && !isRbFullyUnexploredSnapshot(newSnap)){
        ...
        if(shouldRetryBoundary){
          for(let i = 0; i < DBA_BATTLEINFO_RB_BOUNDARY_RETRY_DELAYS_MS.length; i++){
            ...
            // 再取得中に全面未踏マップへ切り替わった場合も、以後の待機は不要。
            // changed は0件のまま、下の通常マップ差し替え処理へ進ませる。
            if(isRbFullyUnexploredSnapshot(newSnap)){
              changed = [];
              break;
            }
      ...
      if(changed.length === 0){
        ...
        const priorityJobs = buildPriorityDetailFetchJobs([], priorityTargets, newSnap);
        if(priorityJobs.length > 0){
          if(btn) btn.textContent = `最新クリック対象を更新中…(${priorityJobs.length})`;
          await mapLimit(priorityJobs, 1, async (job) => {
            const { row, col } = job;
            try{
              const { holder, reg } = await fetchCellDetail(row, col);
              rememberFetchedCellDetailText(newSnap, row, col, holder, reg);
  ...
  async function updateOnlyChangedCellsFromTopPage(){
    if(mode === 'rb'){
      const live = await fetchRbLiveMapSnapshotOnce('battle-info-click');
      if(live && live.snapshot){
        return updateOnlyChangedCellsFromFetchedDoc(null, {
          topUrl: '/map/rb/static',
          snapshot: live.snapshot,
          showAlertOnError: true,
          preferLiveMapFastPath: true
  ...
  function buildSettingsModal(){
    ...
    // ====== セルサイズ調整 グリッドレイアウト ======
    function getCellSizeModeShortLabel(modeKey){
      switch(String(modeKey || '')){
        case 'rb': return 'レッドvsブルー';
        case 'l':  return 'ラダー';
        case 'hc': return 'ハードコア';
        default:   return getModeLabel(modeKey);
      }
    }

    function mkCellSizeHead(text, extraClass){
      const head = document.createElement('div');
      head.className = `dba-setting-cellsize-head${extraClass ? ` ${extraClass}` : ''}`;
      head.textContent = String(text || '');
      return head;
    }

    function mkCellSizeRow(modeKey){
      const frag = document.createDocumentFragment();

      const name = document.createElement('div');
      name.className = 'dba-setting-cellsize-name';
      name.textContent = getCellSizeModeShortLabel(modeKey);

      const sz = getEffectiveCellSizeForMode(modeKey, settings);

      const wWrap = document.createElement('div');
      wWrap.className = 'dba-setting-cellsize-inputwrap';
      ...
      hWrap.className = 'dba-setting-cellsize-inputwrap';
      ...
      resetWrap.className = 'dba-setting-cellsize-reset';
    ...
    (function mkPostBattleAutoDiffBlock(){
      ...
      txtAutoDiff.textContent = '戦闘後に自動でバトルマップを更新する。（画面のちらつきが発生しやすくなります。）';

      labAutoDiff.appendChild(inputAutoDiff);
      labAutoDiff.appendChild(txtAutoDiff);
      body.appendChild(labAutoDiff);

      const autoSyncMode = sanitizePostBattleAutoSyncMode(settings?.postBattle?.autoSyncMode);

      const subModes = document.createElement('div');
      subModes.className = 'dba-setting-subgroup';

      const labAutoMapOnly = document.createElement('label');
      labAutoMapOnly.className = 'dba-setting-radio-line';

      const inputAutoMapOnly = document.createElement('input');
      inputAutoMapOnly.type = 'radio';
      inputAutoMapOnly.name = 'dba-post-battle-auto-sync-mode';
      inputAutoMapOnly.value = 'map';
      inputAutoMapOnly.checked = (autoSyncMode === 'map');
      inputAutoMapOnly.setAttribute('data-post-battle-auto-sync-mode', '1');
      ...
      subModes.appendChild(labAutoMapOnly);

      const labAutoDiffMode = document.createElement('label');
      labAutoDiffMode.className = 'dba-setting-radio-line';
    ...
    (function mkLightRefreshBlock(){
      ...
      noteShowLightBtn.textContent =
        'DBA を含めたページの再読み込みを行って状態を立て直します。ブラウザの「再読み込み」に近い動作ですが、それよりもページを少しきれいな状態で開き直すためのボタンです。動作が重い時、表示がおかしい時、なんとなく不安定な時に使うと、改善することがあります。';
    ...
    (function mkCellSizeBlock(){
      ...
      list.appendChild(mkCellSizeHead('', 'dba-setting-cellsize-head--mode'));
      list.appendChild(mkCellSizeHead('セル幅 px'));
      list.appendChild(mkCellSizeHead('セル高 px'));
      list.appendChild(mkCellSizeHead('数値初期化'));
      list.appendChild(mkCellSizeRow('rb'));
      list.appendChild(mkCellSizeRow('l'));
      list.appendChild(mkCellSizeRow('hc'));
    ...
    function setInputsFromSettings(s){
      ...
      }
      {
        ...
        for(const inp of dlg.querySelectorAll('input[type="radio"][data-post-battle-auto-sync-mode="1"]')){
          inp.checked = (sanitizePostBattleAutoSyncMode(inp.value) === postBattleAutoSyncMode);
        }
      }
      const soff = dlg.querySelector('input[type="checkbox"][data-suppress-own-capital-friendly-fire="1"]');
      if(soff){
  ...
  function injectWhenReady() {
    ...
    if(isArenaPage){
      const doArenaInsert = () => {
        try{
          initArenaPageUtility();
          buildArenaResultModal();
          buildArenaCenterModal();
          buildArenaCenterResultDialog();
          installArenaChallengeResultInterception();
          installArenaCenterInterception();
        }catch(_e){}
      };
      if(document.readyState === 'loading'){
        document.addEventListener('DOMContentLoaded', doArenaInsert, { once:true });
      }else{
        doArenaInsert();
      }
      window.addEventListener('load', () => {
